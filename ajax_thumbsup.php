...
$HTML = '';
$id = (int)$_REQUEST['id'];
($thumbs_query = sql_query("SELECT id, type, torrentid, userid FROM thumbsup WHERE torrentid = " . sqlesc($id))) || sqlerr(__FILE__, __LINE__);
$wtf = $thumbs_query->num_rows;
($res = sql_query("SELECT id, type, torrentid, userid FROM thumbsup WHERE userid = " . sqlesc($CURUSER['id']) . " AND torrentid = " . sqlesc($id))) || sqlerr(__FILE__, __LINE__);
