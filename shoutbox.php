...
if (isset($_GET['del']) && $CURUSER['class'] >= UC_STAFF && is_valid_id($_GET['del'])) {
    sql_query("DELETE FROM shoutbox WHERE id=" . sqlesc($_GET['del'])) || sqlerr(__FILE__, __LINE__);
...
if (isset($_GET['edit']) && $CURUSER['class'] >= UC_STAFF && is_valid_id($_GET['edit'])) {
    ($sql = sql_query('SELECT id, text FROM shoutbox WHERE id=' . sqlesc($_GET['edit']))) || sqlerr(__FILE__, __LINE__);
    $res = $sql->fetch_assoc();
    unset($sql);
...
if (isset($_GET['edit']) && ($_GET['user'] == $CURUSER['id']) && ($CURUSER['class'] >= UC_POWER_USER && $CURUSER['class'] <= UC_STAFF) && is_valid_id($_GET['edit'])) {
    ($sql = sql_query('SELECT id, text, userid FROM shoutbox WHERE userid =' . sqlesc($_GET['user']) . ' AND id=' . sqlesc($_GET['edit']))) || sqlerr(__FILE__, __LINE__);
    $res = $sql->fetch_assoc();
    $HTMLOUT.= "<!DOCTYPE html>
<head>
<script type='text/javascript' src='./scripts/shout.js'></script>
...
if (isset($_GET['sent']) && ($_GET['sent'] == "yes")) {
    ...
    if (preg_match($pattern, $text, $vars) && $CURUSER["class"] >= UC_STAFF) {
        ...
        if ($to_user != 0 && $to_user != $CURUSER['id']) {
            $text = $vars[2] . " - " . $vars[3];
            $text_parsed = format_comment($text);
            sql_query("INSERT INTO shoutbox (userid, date, text, text_parsed, to_user, staff_shout) VALUES (" . sqlesc($userid) . ", $date, " . sqlesc($text) . "," . sqlesc($text_parsed) . "," . sqlesc($to_user) . ", 'no')") || sqlerr(__FILE__, __LINE__);
        ...
        ($a_query = sql_query("SELECT userid, date FROM shoutbox WHERE staff_shout='no' ORDER by id DESC LIMIT 1")) || (print("First shout or an error :)"));
        $a = $a_query->fetch_row();
        if (empty($text) || strlen($text) == 1) {
            ...
            $dailyshouts = 1;
            sql_query("INSERT INTO shoutbox (userid, date, text, text_parsed, staff_shout) VALUES (" . sqlesc($userid) . ", $date, " . sqlesc($text) . "," . sqlesc($text_parsed) . ", 'no')") || sqlerr(__FILE__, __LINE__);
            sql_query("UPDATE usersachiev SET dailyshouts=dailyshouts+1, weeklyshouts = weeklyshouts+1, monthlyshouts = monthlyshouts+1, totalshouts = totalshouts+1 WHERE id= " . sqlesc($userid)) || sqlerr(__FILE__, __LINE__);
            $cache->delete('shoutbox_');
            //$cache->delete('staff_shoutbox_');
            $HTMLOUT.= "<script type=\"text/javascript\">parent.document.forms[0].shbox_text.value='';</script>";
...
if ($shouts && (is_countable($shouts) ? count($shouts) : 0) > 0) {
    $HTMLOUT.= "<table class='small text-left' style='clear:both; border-collapse:collapse; width:100%;'>";
    ($shout_query_a = sql_query(" SELECT count(id) AS pms FROM messages WHERE receiver = " . sqlesc($CURUSER['id']) . " AND unread = 'yes' AND location = '1'")) || sqlerr(__FILE__, __LINE__);
    ...
    if ($shouts) {
        ...
        foreach ($shouts as $arr) {
            ...
            $reply = ($CURUSER['id'] != $arr['userid'] ? "<a href=\"javascript:window.top.SmileIT('[b][i]=>&nbsp;[color=#" . get_user_class_color($arr['class']) . "]" . (($arr['perms'] & bt_options::PERMS_STEALTH) !== 0 ? "UnKnown" : htmlsafechars($arr['username'])) . "[/color]&nbsp;-[/i][/b]','shbox','shbox_text')\"><img height='10' src='{$TRINITY20['pic_base_url']}reply.gif' title='Reply' alt='Reply' style='border:none;' /></a>" : "");
            $user_stuff = $arr;
