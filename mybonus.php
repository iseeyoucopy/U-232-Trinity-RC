	...
	switch ($art) {
		...
		case 'bounty':
			...
			while ($ar = $qr->fetch_assoc()) {
                ...
                //== cache updates ???
				$cache->update_row($keys['my_userid'] . $ar['id'], ['reputation' => $ar['reputation'] - $rep_to_steal], $TRINITY20['expires']['curuser']);
...
the user class is just for show, but what the hell :p Add more or edit to your liking.
			...
			$words = [
				'fuck', 'shit', 'Moderator', 'Administrator', 'Admin', 'pussy', 'Sysop', 'cunt', 'nigger', 'VIP', 'Super User',
				'Power User', 'ADMIN', 'SYSOP', 'MODERATOR', 'ADMINISTRATOR'
			];
