...
($qs = sql_query('SELECT count(t.id) as tickets , u.username, u.id, u.seedbonus FROM tickets as t LEFT JOIN users as u ON u.id = t.user GROUP BY u.id ORDER BY tickets DESC, username ASC')) || sqlerr(__FILE__, __LINE__);
if (!$qs->num_rows) $html.= "<h2>No tickets were bought</h2>";
else {
