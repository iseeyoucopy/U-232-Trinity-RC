...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    foreach ([
        ...
        if (isset($_POST[$key]) && ($type == 0 && $_POST[$key] == 0 || $type == 1 && (is_countable($_POST[$key]) ? count($_POST[$key]) : 0) == 0)) {
    ...
    if (sql_query('INSERT INTO lottery_config(name,value) VALUES '.implode(',', $update).' ON DUPLICATE KEY update value=values(value)')) {
        ...
        stderr('Error', 'There was an error while executing the update query. Mysql error: '.$mysqli->error);
    }
    exit;
}
$html = '';
