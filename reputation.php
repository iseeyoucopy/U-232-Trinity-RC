...
WHERE posts.id =".sqlesc($input['pid']))) || sqlerr(__FILE__, __LINE__);
    ...
    ///////////////////////////////////////////////
    ($forum = sql_query("SELECT id AS userid, username, reputation, opt1, opt2 FROM users WHERE id =".sqlesc($input['pid']))) || sqlerr(__FILE__,
        __LINE__);
} // end
switch ($rep_locale) {
    case 'comments':
...
///////////////////////////////////////////////
($repeat = sql_query("SELECT postid FROM reputation WHERE postid =".sqlesc($input['pid'])." AND whoadded=".sqlesc($CURUSER['id']))) || sqlerr(__FILE__,
    __LINE__);
...
if (!$is_mod) {
    ...
    ///////////////////////////////////////////////
    ($flood = sql_query("SELECT dateadd, userid FROM reputation WHERE whoadded = ".sqlesc($CURUSER['id'])." ORDER BY dateadd DESC LIMIT 0 , ".sqlesc($klimit))) || sqlerr(__FILE__,
        __LINE__);
    if ($flood->num_rows) {
        $i = 0;
