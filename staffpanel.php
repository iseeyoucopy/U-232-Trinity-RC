...
if (($staff_classes = $cache->get($cache_keys['is_staffs'])) === false) {
    ...
    while (($row = $res->fetch_assoc())) {
        ...
        $cache->set($cache_keys['is_staffs'], $staff_classes, 900); //==  test values 900 to 0 with delete keys //==
    }
}
if (!$CURUSER) {
...
if (in_array($tool, $staff_tools) && file_exists(ADMIN_DIR.$staff_tools[$tool].'.php')) {
    ...
    $sure = (($_GET['sure'] ?? '') == 'yes');
    ($res = sql_query('SELECT av_class'.(!$sure || $CURUSER['class'] <= UC_MAX ? ', page_name' : '').' FROM staffpanel WHERE id = '.sqlesc($id))) || sqlerr(__FILE__,
        __LINE__);
    $arr = $res->fetch_assoc();
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        if (empty($errors)) {
            if ($action == 'add') {
                $res = sql_query("INSERT INTO staffpanel (page_name, file_name, description, type, av_class, added_by, added) "."VALUES (".implode(", ",
                        array_map("sqlesc", [
                            $page_name,
                            $file_name,
                            $description,
                ...
                $cache->delete($cache_keys['is_staffs']);
                if (!$res) {
                    $errors[] = $mysqli->errno ? $lang['spanel_this_fname_sub'] : $lang['spanel_db_error_msg'];
                }
            } else {
                ($res = sql_query("UPDATE staffpanel SET page_name = ".sqlesc($page_name).", file_name = ".sqlesc($file_name).", description = ".sqlesc($description).", type = ".sqlesc($type).", av_class = ".sqlesc((int)$av_class)." WHERE id = ".sqlesc($id))) || sqlerr(__FILE__,
                    __LINE__);
                $cache->delete($cache_keys['is_staffs']);
                if (!$res) {
...
<h1 align='center'>{$lang['spanel_welcome']} {$CURUSER['username']} {$lang['spanel_to_the']} {$lang['spanel_header']}!</h1><br>";
    ...
    ($res = sql_query('SELECT staffpanel.*, users.username '.'FROM staffpanel '.'LEFT JOIN users ON users.id = staffpanel.added_by '.'WHERE av_class <= '.sqlesc($CURUSER['class']).' '.'ORDER BY av_class DESC, page_name ASC')) || sqlerr(__FILE__,
        __LINE__);
    if ($res->num_rows > 0) {
