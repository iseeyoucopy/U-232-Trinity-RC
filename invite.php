...
<td align='center'><b>{$lang['invites_confirm']}</b></td>
    ...
    $HTMLOUT .= "</table><br>";
    ($select = sql_query('SELECT * FROM invite_codes WHERE sender = '.sqlesc($CURUSER['id']).' AND status = "Pending"')) || sqlerr(__FILE__,
        __LINE__);
...
if ($do == 'create_invite') {
    ...
    $invite = md5(mksecret());
    sql_query('INSERT INTO invite_codes (sender, invite_added, code) VALUES ('.sqlesc((int)$CURUSER['id']).', '.TIME_NOW.', '.sqlesc($invite).')') || sqlerr(__FILE__,
        __LINE__);
...
elseif ($do == 'send_email') {
    ...
    ($query = sql_query('SELECT * FROM invite_codes WHERE id = '.sqlesc($id).' AND sender = '.sqlesc($CURUSER['id']).' AND status = "Pending"')) || sqlerr(__FILE__,
        __LINE__);
...
elseif ($do == 'delete_invite') {
    ...
    if (!$sure) {
        stderr($lang['invites_delete1'],
            $lang['invites_sure'].' Click <a href="'.$_SERVER['PHP_SELF'].'?do=delete_invite&amp;id='.$id.'&amp;sender='.$CURUSER['id'].'&amp;sure=yes">here</a> to delete it or <a href="?do=view_page">here</a> to go back.');
    }
    sql_query('DELETE FROM invite_codes WHERE id = '.sqlesc($id).' AND sender ='.sqlesc($CURUSER['id'].' AND status = "Pending"')) || sqlerr(__FILE__,
        __LINE__);
    sql_query('UPDATE users SET invites = invites + 1 WHERE id = '.sqlesc($CURUSER['id'])) || sqlerr(__FILE__, __LINE__);
    $update['invites'] = ($CURUSER['invites'] + 1);
    $cache->update_row($keys['my_userid'].$CURUSER['id'], [
...
elseif (($do = 'confirm_account') !== '') {
    ...
    ($select = sql_query('SELECT id, username FROM users WHERE id = '.sqlesc($userid).' AND invitedby = '.sqlesc($CURUSER['id']))) || sqlerr(__FILE__,
        __LINE__);
