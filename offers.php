...
switch ($action) {
    ...
    For breaking the <a class="altlink" href="rules.php">rules</a></form></td>
        ...
        if ($count === 0) {
            ...
            $page = isset($_GET['page']) ? (int)$_GET['page'] : 0;
            $perpage = isset($_GET['perpage']) ? (int)$_GET['perpage'] : 20;
            [$menu, $LIMIT] = pager_new($count, $perpage, $page, 'offers.php?action=offer_details&amp;id='.$id,
                ($perpage == 20 ? '' : '&amp;perpage='.$perpage).'#comments');
            ($subres = sql_query('SELECT c.offer, c.id AS comment_id, c.text, c.added, c.editedby, c.editedat, 
                                    u.id, u.username, u.warned, u.suspended, u.enabled, u.donor, u.class, u.avatar, u.offensive_avatar, u.title, u.leechwarn, u.chatpost, u.pirate, u.king FROM comments AS c LEFT JOIN users AS u ON c.user = u.id WHERE c.offer = '.sqlesc($id).' ORDER BY c.id '.$LIMIT)) || sqlerr(__FILE__,
                __LINE__);
            $allrows = [];
   ...
   '.(isset($_POST['button']) && $_POST['button'] == 'Preview' ? '<br>
    ...
    case 'edit_offer':
        ...
        ($edit_res = sql_query('SELECT offer_name, image, description, category, offered_by_user_id, link FROM offers WHERE id ='.sqlesc($id))) || sqlerr(__FILE__,
            __LINE__);
    ...
    case 'edit_comment':
        ...
        ($res = sql_query('SELECT c.*, o.offer_name FROM comments AS c LEFT JOIN offers AS o ON c.offer = o.id WHERE c.id='.sqlesc($comment_id))) || sqlerr(__FILE__,
            __LINE__);
        $arr = $res->fetch_assoc();
        if (!$arr) {
            stderr('Error', 'Invalid ID.');
