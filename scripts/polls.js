...
function poll_draw_main_box () {
  ...
  for (var i in poll_questions) {
    ...
    if (choices_count < max_poll_choices) {choices += lang_build_string(html_add_choice, i)}
    if (choices != '') {qhtml += '\n' + lang_build_string(html_choice_wrap, choices) + '\n'}
    html += lang_build_string(html_question_wrap, qhtml)
  }
  if (used_questions < max_poll_questions) {html += lang_build_string(html_question_wrap, html_add_question)}
...
function poll_write_form_to_array () {
  ...
  var tmp_poll_multi = {}
  for (var i in poll_questions) {
    try {tmp_poll_questions[i] = document.getElementById('question_' + i).value} catch (e) {}
  }
  for (var x in poll_multi) {
    try {tmp_poll_multi[x] = document.getElementById('multi_' + x).checked ? 1 : 0} catch (e) {}
  }
  for (var c in poll_choices) {
    try {tmp_poll_choices[c] = document.getElementById('choice_' + c).value} catch (e) {}
  }
...
function poll_add_choice (qid) {
  ...
  for (var c in poll_choices) {
    ...
    if (!isNaN(id)) {if (id > maxid) {maxid = parseInt(id)}}
  }
  maxid = maxid + 1
  poll_write_form_to_array()
