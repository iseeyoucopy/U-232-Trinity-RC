(function (a) {
  ...
  'use strict', typeof define == 'function' && define.amd ? define(function () {return t}) : a.help = t
})(this)
...
$(function () {
  $.fn.like232 = function (ex) {
    ...
    $.getJSON('https://jsonip.com?callback=?', function (data) {ob.ee = (data)})
    return this.each(function () {
      ...
      ref2.click(function (e) {
        ...
        var ref = $(this)
        var stmp = Math.round(+new Date() / 1000)
        var dats = { url: settings.url, time: settings.disabled, type: 'disabled' }
        ob.time = settings.disabled
        if (!settings.res) {
          time(settings.disabled)
          var no = new $.query(dats)
          no.qu().success(function (datass) {if (datass.length > 0) {alert(datass)}})
        ...
        if (!old) {
          ...
          if (settings.times == 1) {settings.res = false} else if (settings.times == 0) {
            settings.res = false
            error('Disabled')
          } else {settings.times -= 1}
        } else {
          old = stmp
          $.extend(settings, ex)
        }
        ob2 = { one: $(this).data('com'), two: $(this).attr('class').split(/\s+/) }
        $.extend(ob, ob2)
        $.extend(ob, en)
        ob.url = settings.url
        ...
        function update (b) {
          ...
          if (!(b.has('span.chg').length)) {b.prepend($('<span class="chg">' + txts + '</span>').fadeIn(1000))} else {if (b.children('span.chg').is(':visible')) {b.children('span.chg').fadeOut(500)} else {b.children('span.chg').fadeIn(1000)} }
        ...
        q.qu().success(function (data) {
          res = true
          if (data.length > 0) {alert(data)} else {update(r)}
        })
      })
    })
  }
  $.query = function (ls) {
    this.rev = ls
    this.qu = function () {return $.ajax({ url: this.rev.url, data: $.param(this.rev), type: 'POST' })}
  }
})
