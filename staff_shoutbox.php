...
if ((isset($_GET['show_staffshout'])) && (($show_shout = htmlsafechars($_GET['show_staff'])))) {
    ...
    $mc1->commit_transaction($TRINITY20['expires']['user_cache']);
    ...
    if (preg_match($pattern, $text, $vars) && $CURUSER["class"] >= UC_STAFF) {
        ...
        if (mysqli_num_rows($c) == 1 && $CURUSER["class"] > $a[1]) {
            ...
            case "/GAG":
                ...
                $mc1->commit_transaction($TRINITY20['expires']['curuser']);
                ...
                $mc1->commit_transaction($TRINITY20['expires']['user_stats']);
                break;
...
if (($shouts = $mc1->get_value('staff_shoutbox_')) === false) {
    ...
    $mc1->cache_value('staff_shoutbox_', $shouts, $TRINITY20['expires']['staff_shoutbox']);
}
if (count($shouts) > 0) {
