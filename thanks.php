...
function print_list()
{
    ...
    ($qt = sql_query("SELECT th.userid, u.username, u.seedbonus FROM thanks as th INNER JOIN users as u ON u.id=th.userid WHERE th.torrentid=".sqlesc($tid)." ORDER BY u.class DESC")) || sqlerr(__FILE__,
        __LINE__);
    $list = [];
    $hadTh = false;
    if ($qt->num_rows > 0) {
...
switch ($do) {
    ...
    case 'add':
        ...
        if ($TRINITY20['seedbonus_on'] == 1) {
            // ===add karma
            sql_query("UPDATE users SET seedbonus = seedbonus+".sqlesc($TRINITY20['bonus_per_thanks'])." WHERE id =".sqlesc($uid)) || sqlerr(__FILE__,
                __LINE__);
