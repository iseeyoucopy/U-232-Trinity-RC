...
class GeSHi
{
     ...
     * @var array
     */
    protected $highlight_extra_lines = [];
     ...
     * @var array
     */
    protected $highlight_extra_lines_styles = [];
     ...
     * @since 1.0.5
     */
    public static function get_language_name_from_extension($extension, $lookup = [])
    {
        ...
        if (!is_array($lookup) || empty($lookup)) {
            $lookup = [
                ...
                'visualfoxpro' => [],
                'whitespace' => ['ws'],
                'xml' => ['xml', 'svg', 'xrc', 'vbproj', 'csproj', 'userprefs', 'resx', 'stetic', 'settings', 'manifest', 'myapp'],
                'z80' => ['z80', 'asm', 'inc'],
            ];
        }
...
//        $result = substr($result, 1, -1);
    ...
    protected function parse_non_string_part($stuff_to_parse)
    {
            ...
            foreach ($this->language_data['OBJECT_SPLITTERS'] as $key => $splitter) {
                if (false !== strpos($stuff_to_parse, $splitter)) {
                    ...
                    $stuff_to_parse = preg_replace("/($oolang_before)(".preg_quote($this->language_data['OBJECT_SPLITTERS'][$key],
                            '/').")($oolang_spaces)($oolang_after)/", "\\1\\2\\3<|$attributes>\\4|>", $stuff_to_parse);
                }
            }
        }
        ...
        $stuff_to_parse = str_replace(['<|', '|>'], ['<span', '</span>'], $stuff_to_parse);
