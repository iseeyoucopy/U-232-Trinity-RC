    ...
    while ($row = $res->fetch_assoc()) {
        ...
        if ($CURUSER['opt2'] & user_options_2::SPLIT && $row['sticky'] == 'no' && $variant == 'index') {
            ...
            if (get_date($row['added'], 'DATE') == $prevdate) {
                ...
                $htmlout .= "<thead><tr><th class='text-left' colspan='".(($CURUSER['class'] >= UC_STAFF) || ($CURUSER['class'] < UC_VIP) ? 13 : 12)."'><b>{$lang['torrenttable_upped']} ".get_date($row['added'],
                        'DATE')."</b></th></tr></thead>";
            }
        ...
        */
        ...
        if (!empty($row['newgenre'])) {
            ...
            foreach ($row['newgenre'] as $foo) {
                $newgenre[] = '<a href="browse.php?search='.trim(strtolower($foo)).'&amp;searchin=genre&amp;incldead=0">'.$foo.'</a>';
            }
        ...
        if (!empty($row['tags'])) {
            ...
            foreach ($row['tags'] as $boo) {
                $tags[] = '<a href="browse.php?search='.trim(strtolower($boo)).'&amp;searchin=tags&amp;incldead=0">'.$boo.'</a>';
            }
        ...
        if (XBT_TRACKER == true) {
            ...
            $freetorrent = ($row["free"] != 0 ? "<span class='label secondary' style='margin-left: 6px;padding: 2px 4px;border-radius:2px;' title='".($row['free'] > 1 ? "Expires: ".get_date($row['free'],
                        'DATE')."&nbsp;(".mkprettytime($row['free'] - TIME_NOW)." to go)'>FREELEECH</span>" : "Unlimited'>FREELEECH</span>") : "");
        }
        /** Silver Torrent **/
        ...
        /*Added*/
        $htmlout .= "<td class='text-center' style='padding-top: 0;padding-right: 0;padding-bottom: 0;padding-left: 0;'>".str_replace(",", "<br>",
                get_date($row['added'], 'TTABLE'))."</td>";
        if ($CURUSER["class"] < UC_VIP && $TRINITY20['wait_times'] == 1) {
            /*wait times*/
        ...
        /*size*/
        $htmlout .= "<td class='text-center' style='padding-top: 0;padding-right: 0;padding-bottom: 0;padding-left: 0;'>".str_replace(" ", "&nbsp;",
                mksize($row["size"]))."</td>";
        /*snatches*/
