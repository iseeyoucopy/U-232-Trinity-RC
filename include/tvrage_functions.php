...
function tvrage_format($tvrage_data, $tvrage_type)
{
    ...
    foreach ($tvrage_data as $key => $data) {
        ...
        $tvrage_display[$tvrage_type][$key] = is_string($data) ? sprintf($tvrage_display[$tvrage_type][$key],
            $data) : sprintf($tvrage_display[$tvrage_type][$key], $data['number'], $data['title'], date('M/d/Y - l', strtotime($data['airdate'])),
...
function tvrage(&$torrents)
{
    ...
    if ($force_update || ($tvrage_showinfo = $cache->get($memkey)) === false) {
        ...
        $tvrage_xml = file_get_contents($tvrage_link);
        preg_match_all('/\<(showname|showlink|startdate|ended|image|origin_country|status|classification|summary|airtime|runtime)\>(.+)\<\/\\1\>/s',
            $tvrage_xml, $tmp, PREG_SET_ORDER);
        foreach ($tmp as $data) {
            if (!$data[2]) {
                continue;
            }
        ...
        if ((is_countable($tmp[1]) ? count($tmp[1]) : 0) > 0) {
            $tvrage_showinfo['genres'] = implode(', ', array_map('strtolower', $tmp[1]));
        }
        ...
        if ((is_countable($row_update) ? count($row_update) : 0) > 0) {
            sql_query('UPDATE torrents set '.implode(', ',
                    $row_update).' WHERE id = '.$torrents['id']) || sqlerr(__FILE__, __LINE__);
        }
    ...
    if ($tvrage['season'] > 0 && $tvrage['episode'] > 0) {
        ...
        if (($tvrage_epinfo = $cache->get($memkey)) === false) {
            ...
            //get episode info
            $tvrage_link = sprintf('http://services.tvrage.com/myfeeds/episodeinfo.php?key=%s&sid=%d&ep=%dx%d', $TRINITY20['tvrage_api'], $tvrage_id,
                $tvrage['season'], $tvrage['episode']);
            $tvrage_xml = file_get_contents($tvrage_link);
