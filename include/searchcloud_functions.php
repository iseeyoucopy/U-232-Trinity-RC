...
function searchcloud_insert($word)
{
    ...
    sql_query('INSERT INTO searchcloud(searchedfor,howmuch,ip) VALUES (' . sqlesc($word) . ',1,' . sqlesc($ip) . ') ON DUPLICATE KEY UPDATE howmuch=howmuch+1') || sqlerr(__FILE__, __LINE__);
}
function cloud()
{
    //min / max font sizes
