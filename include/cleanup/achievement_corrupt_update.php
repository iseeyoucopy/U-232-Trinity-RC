...
function docleanup($data)
{
    ...
    ($res = sql_query("SELECT users.id, users.corrupt, usersachiev.corrupt FROM users LEFT JOIN usersachiev ON users.id = usersachiev.id WHERE enabled = 'yes' AND users.corrupt >= '1' AND usersachiev.corrupt = '0'")) || sqlerr(__FILE__, __LINE__);
    ...
    if ($res->num_rows > 0) {
        ...
        if ($count > 0) {
            ...
            sql_query("INSERT INTO usersachiev (id, corrupt, achpoints) VALUES " . implode(', ', $usersachiev_buffer) . " ON DUPLICATE key UPDATE corrupt=values(corrupt), achpoints=achpoints+values(achpoints)") || sqlerr(__FILE__, __LINE__);
