...
function docleanup($data)
{
    ...
    ($res = sql_query("SELECT id, modcomment FROM users WHERE warned > 1 AND warned < " . TIME_NOW)) || sqlerr(__FILE__, __LINE__);
    ...
    if ($res->num_rows > 0) {
        ...
        if ($count > 0) {
            ...
            sql_query("INSERT INTO users (id, warned, modcomment) VALUES " . implode(', ', $users_buffer) . " ON DUPLICATE key UPDATE warned=values(warned), modcomment=values(modcomment)") || sqlerr(__FILE__, __LINE__);
