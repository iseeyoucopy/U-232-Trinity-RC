...
function docleanup($data)
{
    ...
    ($res = sql_query("SELECT id, username, class, donor, title, warned, enabled, chatpost, leechwarn, pirate, king, uploaded, birthday FROM users WHERE MONTH(birthday) = ".sqlesc($current_date['mon'])." AND DAYOFMONTH(birthday) = ".sqlesc($current_date['mday'])." ORDER BY username ASC")) || sqlerr(__FILE__,
        __LINE__);
    ...
    if ($res->num_rows > 0) {
        ...
        if ($count > 0) {
            ...
            sql_query("INSERT INTO users (id, uploaded) VALUES ".implode(', ',
                    $users_buffer)." ON DUPLICATE key UPDATE uploaded=uploaded+values(uploaded)") || sqlerr(__FILE__, __LINE__);
