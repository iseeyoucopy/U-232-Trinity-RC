...
{
    global $cache, $TRINITY20, $mysqli, $keys;
    $crazy_hour = (TIME_NOW + 3600);
    if (($cz['crazyhour'] = $cache->get($keys['crazyhour'])) === false) {
        ($cz['sql'] = ann_sql_query('SELECT var, amount FROM freeleech WHERE type = "crazyhour"')) || ann_sqlerr(__FILE__, __LINE__);
        $cz['crazyhour'] = [];
        if ($cz['sql']->num_rows !== 0) {
            $cz['crazyhour'] = $cz['sql']->fetch_assoc();
        } else {
        ...
        $cache->set($keys['crazyhour'], $cz['crazyhour'], 0);
    }
    if ($cz['crazyhour']['var'] < TIME_NOW) { // if crazyhour over
        if (($cz_lock = $cache->set($keys['crazyhour_lock'], 1, 10)) !== false) {
...
function freeleech_announce()
{
    ...
    if (($fl['countdown'] = $cache->get($keys['freeleech_countdown'])) === false) { // pot_of_gold
        ...
        $cache->set($keys['freeleech_countdown'], $fl['countdown'], 0);
    }

    if (($fl['countdown']['var'] !== 0) && (TIME_NOW > ($fl['countdown']['var']))) { // end of freeleech sunday
        if (($fc_lock = $cache->set($keys['freeleech_countdown_lock'], 1, 10)) !== false) {
            $fl['countdown']['var'] = 0;
            //$fl['countdown']['amount'] = strtotime('next Monday');  // timestamp sunday
            $fl['countdown']['amount'] = 43200;  // timestamp test
            ann_sql_query('UPDATE LOW_PRIORITY freeleech SET var = '.ann_sqlesc($fl['countdown']['var']).', amount = '.ann_sqlesc($fl['countdown']['amount']).' '.
                'WHERE type = "countdown"') || ann_sqlerr(__FILE__, __LINE__);
            $cache->update_row($keys['freeleech_countdown'], ['var' => $fl['countdown']['var'], 'amount' => $fl['countdown']['amount']], 0);
        }
        return false;
    }

    if (TIME_NOW > ($fl['countdown']['amount'])) {
        if ($fl['countdown']['var'] == 0 && ($cz_lock = $cache->set($keys['crazyhour_lock'], 1, 10)) !== false) {
            ...
            ann_sql_query('UPDATE LOW_PRIORITY freeleech SET var = '.ann_sqlesc($fl['countdown']['var']).' '.
                'WHERE type = "countdown"') || ann_sqlerr(__FILE__, __LINE__);
            $cache->update_row($keys['freeleech_countdown'], ['var' => $fl['countdown']['var']], 0);
...
function get_user_from_torrent_pass($torrent_pass)
{
    ...
    if (($user = $cache->get($keys['user_torrent_pass'].$torrent_pass)) === false) {
        $user_fields_ar_int = [
            'id',
            'uploaded',
            'downloaded',
        ...
        $cache->set($keys['user_torrent_pass'].$torrent_pass, $user, $TRINITY20['expires']['user_passkey']);
    } elseif (!$user) {
        return false;
    }
    return $user;
...
function get_torrent_from_hash($info_hash)
{
    global $cache, $keys;
    $ttll = 21600; // 21600;
    if (($torrent = $cache->get($keys['torrent_hash'].md5($info_hash))) === false) {
        ...
        if ($res->num_rows) {
            ...
            $cache->set($keys['torrent_hash'].md5($info_hash), $torrent, $ttll);
            $cache->set($keys['torrents_seeds'].$torrent['id'], $torrent['seeders'], $ttll);
            $cache->set($keys['torrents_leechs'].$torrent['id'], $torrent['leechers'], $ttll);
            $cache->set($keys['torrents_comps'].$torrent['id'], $torrent['times_completed'], $ttll);
        } else {
            $cache->set($keys['torrent_hash'].md5($info_hash), 0, 86400);
            return false;
        }
    } elseif (!$torrent) {
        return false;
    } else {
        $torrent['seeders'] = $cache->get($keys['torrents_seeds'].$torrent['id']);
        $torrent['leechers'] = $cache->get($keys['torrents_leechs'].$torrent['id']);
        $torrent['times_completed'] = $cache->get($keys['torrents_comps'].$torrent['id']);
        if ($torrent['seeders'] === false || $torrent['leechers'] === false || $torrent['times_completed'] === false) {
            ...
            if ($res->num_rows) {
                ...
                $cache->delete($keys['torrent_hash'].md5($info_hash));
                return false;
            }
        }
    }
...
function adjust_torrent_peers($id, $seeds = 0, $leechers = 0, $completed = 0)
{
    ...
    $adjust = 0;
    if ($seeds > 0) {
        $adjust += (bool)$cache->increment($keys['torrents_seeds'].$id, $seeds);
    } elseif ($seeds < 0) {
        $adjust += (bool)$cache->decrement($keys['torrents_seeds'].$id, -$seeds);
    }
    if ($leechers > 0) {
        $adjust += (bool)$cache->increment($keys['torrents_leechs'].$id, $leechers);
    } elseif ($leechers < 0) {
        $adjust += (bool)$cache->decrement($keys['torrents_leechs'].$idy, -$leechers);
    }
    if ($completed > 0) {
        $adjust += (bool)$cache->increment($keys['torrents_comps'], $completed);
    }
    return (bool)$adjust;
}
// happyhour by putyn
...
function get_slots($torrentid, $userid)
{
    ...
    if (($slot = $cache->get($keys['fllslot'].$userid)) === false) {
        ...
        $cache->set($keys['fllslot'].$userid, $slot, $ttl_slot);
    }
    if (!empty($slot)) {
        foreach ($slot as $sl) {
