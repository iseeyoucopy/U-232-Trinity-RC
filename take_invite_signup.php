...
if ($arr[0] >= $TRINITY20['invites']) {
    stderr($lang['stderr_errorhead'], sprintf($lang['stderr_ulimit'], $TRINITY20['invites']));
}
if (!$TRINITY20['openreg_invites']) {
    stderr('Sorry', 'Invite Signups are closed presently');
}
if (!mkglobal('wantusername:wantpassword:passagain:email:invite'.($TRINITY20['captcha_on'] ? ":captchaSelection:" : ":").'submitme:passhint:hintanswer')) {
...
($new_user = sql_query("INSERT INTO users (username, passhash, hash3, secret, passhint, hintanswer, editsecret, birthday, country, gender, stylesheet, invitedby, email, added, last_access, last_login, time_offset, dst_in_use, free_switch, pin_code) VALUES (".implode(",",
        array_map("sqlesc", [
            $wantusername,
            $wantpasshash,
            $hash3,
...
sql_query("INSERT INTO messages (sender, subject, receiver, msg, added) VALUES (0, $subject, ".sqlesc($sender).", $msg, $added)") || sqlerr(__FILE__,
    __LINE__);
$cache->delete($cache_keys['inbox_new'].$sender);
$cache->delete($cache_keys['inbox_new_sb'].$sender);
//////////////end/////////////////////
