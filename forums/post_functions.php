...
<td class='thead' colspan='2'><strong>Compose</strong></td>
    ...
    if ($quote) {
        ...
        ($res = sql_query("SELECT posts.*, users.username FROM posts JOIN users ON posts.user_id = users.id WHERE posts.id =".sqlesc($postid))) || sqlerr(__FILE__,
            __LINE__);
        if ($res->num_rows == 0) {
            ...
            while ($post = $postres->fetch_assoc()) {
                ...
                if ($post["anonymous"] == "yes") {
                    if ($CURUSER['class'] < UC_STAFF && $post["uid"] != $CURUSER["id"]) {
                        ...
                        $htmlout .= "<p class='sub'>#".(int)$post["id"]." by <i>Anonymous</i> [<b>".format_username($user_stuff,
                                true)."</b>] at ".get_date($post["added"], 'LONG', 1, 0)."</p>";
                    }
                } else {
                    $htmlout .= "<p class='sub'>#".(int)$post["id"]." by ".(empty($post["username"]) ? "unknown[".(int)$post['uid']."]" : format_username($user_stuff,
                            true))." at ".get_date($post["added"], 'LONG', 1, 0)."</p>";
                }
                $htmlout .= begin_table(true);
                $htmlout .= "<tr>
