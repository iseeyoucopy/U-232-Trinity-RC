...
if (!defined('IN_TRINITY20_FORUM')) {
    ...
    ($res = sql_query("SELECT pp.id, t.id AS tid FROM postpolls AS pp LEFT JOIN topics AS t ON t.poll_id = pp.id WHERE pp.id=" . sqlesc($pollid))) || sqlerr(__FILE__, __LINE__);
    if ($res->num_rows == 0) {
