...
function wikimenu()
{
    ...
    $latestarticle = articlereplace(htmlspecialchars($latest["name"]));
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_POST["article-add"])) {
        $name = htmlspecialchars($_POST["article-name"]);
        $body = htmlspecialchars($_POST["article-body"]);
...
VALUES (".sqlesc($name).", ".sqlesc($body).", ".sqlesc($CURUSER["id"]).", '".TIME_NOW."')") || sqlerr(__FILE__, __LINE__);
        $HTMLOUT .= "<meta http-equiv=\"refresh\" content=\"0; url=wiki.php?action=article&name=".htmlspecialchars($_POST["article-name"])."\">";
    }
    ...
    if (isset($_POST["wiki"])) {
        $wikisearch = articlereplace(htmlspecialchars($_POST["article"]));
  ...
  <div id=\"wiki-row\">";
        while ($wiki = $res->fetch_array(MYSQLI_BOTH)) {
            if ($wiki['lastedit']) {
                ...
                $edit = "<i>Last Updated by: <a href=\"userdetails.php?id=".(int)$wiki['userid']."\">".htmlspecialchars($checkit['username'])."</a> - ".datetimetransform($wiki['lastedit'])."</i>";
            }
            ...
            $HTMLOUT .= "
				<div id=\"wiki-content-left\" align=\"right\">
					<div id=\"name\"><b><a href=\"wiki.php?action=article&amp;name=".htmlspecialchars($wiki['name'])."\">".htmlspecialchars($wiki['name'])."</a></b></div>
					<div id=\"content\">".($wiki['userid'] > 0 ? "<i>{$lang['wiki_added_by_art']}<a href=\"userdetails.php?id=".(int)$wiki['userid']."\"><b>".htmlspecialchars($author['username'])."</b></a></i><br /><br />" : "").wikireplace(format_comment($wiki["body"]))."";
	...
	<b><a href=\"wiki.php?action=article&amp;name=".articlereplace(htmlspecialchars($wiki["name"]))."\">".htmlspecialchars($wiki['name'])."</a></b>{$lang['wiki_added_by']}<a href=\"userdetails.php?id=".(int)$wiki['userid']."\">".htmlspecialchars($wikiname['username'])."</a></div>";
            }
        } else {
...
if ($action == "add") {
    ...
    $HTMLOUT .= "<div id=\"wiki-content-left\">
					...
					<div id=\"content-add\"><textarea name=\"article-body\" rows=\"70\" cols=\"90\" id=\"body\">".htmlspecialchars($wiki['body'])."</textarea>
        ...
        while ($wiki = $res->fetch_array(MYSQLI_BOTH)) {
            $HTMLOUT .= "
				<div id=\"wiki-content-left\" align=\"right\">
					...
					<input type=\"text\" name=\"article-name\" id=\"name\" value=\"".htmlspecialchars($wiki['name'])."\" /></div>
					<div id=\"content-add\"><table class=\"table table bordered\"><tr><td><textarea name=\"article-body\" rows=\"70\" cols=\"10\" id=\"body\">".htmlspecialchars($wiki['body'])."</textarea>
...
if ($action == "sort") {
    ...
    if ($sortres->num_rows > 0) {
        $HTMLOUT .= navmenu();
        $HTMLOUT .= "{$lang['wiki_art_found_starting']}<b>".htmlspecialchars($letter)."</b>";
        while ($wiki = $sortres->fetch_array(MYSQLI_ASSOC)) {
            ...
            $HTMLOUT .= "
				<div class=\"wiki-search\">
					<b><a href=\"wiki.php?action=article&amp;name=".articlereplace(htmlspecialchars($wiki["name"]))."\">".htmlspecialchars($wiki['name'])."</a></b>{$lang['wiki_added_by1']}<a href=\"userdetails.php?id=".(int)$wiki['userid']."\">".htmlspecialchars($wikiname['username'])."</a></div>";
        }
    } else {
        $HTMLOUT .= navmenu();
