...
$potsize = 50000;
/** Site Pot **/
($Pot_query = sql_query("SELECT value_s, value_i, value_u FROM avps WHERE arg = 'sitepot'")) || sqlerr(__file__, __line__);
($SitePot = $Pot_query->fetch_assoc()) || stderr('ERROR', 'db error.');
if ($SitePot['value_u'] < TIME_NOW && $SitePot['value_s'] == '1') {
    sql_query("UPDATE avps SET value_i = 0, value_s = '0' WHERE arg = 'sitepot'") || sqlerr(__file__, __line__);
...
//=== get total points
if (($site_pot_counter = $cache->get($cache_keys['site_pot_counter'])) === false) {
    ...
    $cache->set($cache_keys['site_pot_counter'], $percent);
} else {
...
if ($SitePot['value_i'] == $potsize) {
    stderr('Site Pot is Full',
        'Freeleech ends at: '.get_date($SitePot['value_u'], 'DATE').' ('.mkprettytime($SitePot['value_u'] - TIME_NOW).' to go).');
}
...
if ($want_pot && (isset($pot_options[$want_pot]))) {
    ...
    if ($potsize != $SitePot['value_i'] + $want_pot) {
        ...
        $cache->delete($cache_keys['site_pot_counter']);
        /** shoutbox announce **/
