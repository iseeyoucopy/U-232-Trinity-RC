...
if (happyHour('check') && happyCheck('checkid', $row['category']) && XBT_TRACKER == false && $TRINITY20['happy_hour'] == true) {
    ...
    sql_query('INSERT INTO happyhour (userid, torrentid, multiplier ) VALUES (' . sqlesc($CURUSER['id']) . ',' . sqlesc($id) . ',' . sqlesc($multiplier) . ')') || sqlerr(__FILE__, __LINE__);
...
if ($TRINITY20['seedbonus_on'] == 1 && $row['owner'] != $CURUSER['id']) {
    //===remove karma
    sql_query("UPDATE users SET seedbonus = seedbonus-".sqlesc($TRINITY20['bonus_per_download'])." WHERE id = " . sqlesc($CURUSER["id"])) || sqlerr(__FILE__, __LINE__);
