...
if ($member) {
    ($res_username = sql_query('SELECT id, username, class, warned, suspended, leechwarn, chatpost, pirate, king, enabled, donor FROM users WHERE LOWER(username)=LOWER('.sqlesc($member).') LIMIT 1')) || sqlerr(__FILE__,
        __LINE__);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    switch (true) {
        ...
        case (!$keywords && $member):
            ($res_search = sql_query("SELECT * FROM messages WHERE sender = ".sqlesc($arr_username['id'])." AND saved = 'yes' $location AND receiver = ".sqlesc($CURUSER['id'])." ORDER BY ".sqlesc($sort)." $desc_asc LIMIT ".$limit)) || sqlerr(__FILE__,
                __LINE__);
            break;
        //=== if system entered default both ...

        case (!$keywords && $member_sys):
            ($res_search = sql_query("SELECT * FROM messages WHERE sender = 0 $location AND receiver = ".sqlesc($CURUSER['id'])." ORDER BY ".sqlesc($sort)." $desc_asc LIMIT ".$limit)) || sqlerr(__FILE__,
                __LINE__);
            break;
        //=== if just subject

        case ($subject && !$text):
            ($res_search = sql_query("SELECT *, MATCH(subject) AGAINST(".sqlesc($search)." IN BOOLEAN MODE) AS relevance FROM messages WHERE ( MATCH(subject) AGAINST (".sqlesc($search)." IN BOOLEAN MODE) ) $and_member $location $what_in_out ORDER BY ".sqlesc($sort)." $desc_asc LIMIT ".$limit)) || sqlerr(__FILE__,
                __LINE__);
            break;
        //=== if just message

        case (!$subject && $text):
            ($res_search = sql_query("SELECT *, MATCH(msg) AGAINST(".sqlesc($search)." IN BOOLEAN MODE) AS relevance FROM messages WHERE ( MATCH(msg) AGAINST (".sqlesc($search)." IN BOOLEAN MODE) ) $and_member $location $what_in_out ORDER BY ".sqlesc($sort)." $desc_asc LIMIT ".$limit)) || sqlerr(__FILE__,
                __LINE__);
            break;
        //=== if subject and message

        case ($subject && $text || !$subject && !$text):
            ($res_search = sql_query("SELECT *, ( (1.3 * (MATCH(subject) AGAINST (".sqlesc($search)." IN BOOLEAN MODE))) + (0.6 * (MATCH(msg) AGAINST (".sqlesc($search)." IN BOOLEAN MODE)))) AS relevance FROM messages WHERE ( MATCH(subject,msg) AGAINST (".sqlesc($search)." IN BOOLEAN MODE) ) $and_member $location $what_in_out ORDER BY ".sqlesc($sort)." $desc_asc LIMIT ".$limit)) || sqlerr(__FILE__,
                __LINE__);
            break;
    }
    ...
    while ($row = $res_search->fetch_assoc()) {
        ...
        if (!$member) {
            ($res_username = sql_query('SELECT id, username, warned, suspended, enabled, donor, leechwarn, chatpost, pirate, king, class FROM users WHERE id = '.sqlesc($row[$sender_reciever]).' LIMIT 1')) || sqlerr(__FILE__,
                __LINE__);
            $arr_username = $res_username->fetch_assoc();
            $the_username = print_user_stuff($arr_username);
        }
    ...
    <tr>
        ...
        '.($all_boxes ? $lang['pm_search_foundin'].$arr_box.$lang['pm_search_sign'] : '').$lang['pm_search_at'].get_date($row['added'],
                '').$lang['pm_search_gmt'].get_date($row['added'], '', 0, 1).''.$lang['pm_search_sign'].'</td>
    </tr>
    <tr>
