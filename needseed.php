...
$HTMLOUT.="<div class='row'><div class='col-md-12'><h2>{$lang['needseed_sin']}&nbsp;&nbsp;-&nbsp;&nbsp;[<a href='?needed=leechers' class='altlink'>{$lang['needseed_tns']}</a>]</h2></div></div>";
    $HTMLOUT.="<div class='row'><div class='col-md-12'>";
    ($res = sql_query("SELECT id, name, seeders, leechers, added, category FROM torrents WHERE leechers >= 0 AND seeders = 0 ORDER BY leechers DESC LIMIT 20")) || sqlerr(__FILE__, __LINE__);
    if ($res->num_rows > 0) {
        $HTMLOUT.= "<table class='table table-bordered'>
