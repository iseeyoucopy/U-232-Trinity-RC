...
if ($TRINITY20['hnr_online'] == 1 || $CURUSER['id'] == $id || $CURUSER['class'] >= UC_POWER_USER) {
    ...
    <td align='center' class='$class'>".($a['downloaded'] > 0 ? "<font color='".get_ratio_color(number_format($a['uploaded'] / $a['downloaded'],
    ...
    <td align='center' class='$class'>".($a['downloaded'] > 0 ? "<font color='".get_ratio_color(number_format($a['uploaded'] / $a['downloaded'],
                            ...
                            3)."</font>" : ($a['uploaded'] > 0 ? $lang['userdetails_c_inf'] : '---'))."<br /></td>
    ...
    if ($completed && $CURUSER['class'] >= UC_POWER_USER || $completed && $user['id'] == $CURUSER['id']) {
        if (!isset($_GET['completed'])) {
            $HTMLOUT .= tr('<b>'.$lang['userdetails_completedt'].'</b><br />',
                '[ <a href=\'./userdetails.php?id='.$id.'&amp;completed=1#completed\' class=\'sublink\'>'.$lang['userdetails_c_show'].'</a> ]&nbsp;&nbsp;-&nbsp;'.$r->num_rows,
                1);
        } elseif ($r->num_rows == 0) {
            $HTMLOUT .= tr('<b>'.$lang['userdetails_completedt'].'</b><br />',
                '[ <a href=\'./userdetails.php?id='.$id.'&amp;completed=1\' class=\'sublink\'>'.$lang['userdetails_c_show'].'</a> ]&nbsp;&nbsp;-&nbsp;'.$r->num_rows,
                1);
        } else {
            $HTMLOUT .= tr('<a name=\'completed\'><b>'.$lang['userdetails_completedt'].'</b></a><br />[ <a href=\'./userdetails.php?id='.$id.'#history\' class=\'sublink\'>'.$lang['userdetails_c_hide'].'</a> ]',
                $completed, 1);
        }
    }
}
