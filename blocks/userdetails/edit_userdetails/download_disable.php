...
if ($user['downloadpos'] != 1) {
    ...
    if ($user['downloadpos'] == 1) {
        ...
        $HTMLOUT .= "<p class='help-text' id='dload_pos'>{$lang['userdetails_until']} ".get_date($user['downloadpos'],
                'DATE')." (".mkprettytime($user['downloadpos'] - TIME_NOW)." {$lang['userdetails_togo']})</p>";
    }
