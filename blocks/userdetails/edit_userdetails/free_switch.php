...
$HTMLOUT.= "<div class='input-group'>
    ...
    if ($user['free_switch'] == 1) 
        ...
        $HTMLOUT.= "<p class='help-text' id='freeswitch'>{$lang['userdetails_freeleech_status']} {$lang['userdetails_until']} " . get_date($user['free_switch'], 'DATE') . " (" . mkprettytime($user['free_switch'] - TIME_NOW) . " {$lang['userdetails_togo']})</p>";
}
if ($user['free_switch'] == 0) {
    $HTMLOUT.= '<div class="input-group">
