    ...
    while ($arr = $res->fetch_assoc()) {
        ...
        $htmlout.= "<div class='divTableBody'>
            <div class='divTableRow'>
                ...
                <div class='divTableCell'><a href='details.php?id=" . (int)$XBT_or_PHP . "'><b>" . (strlen($arr["name"]) > 50 ? substr($arr["name"], 0, 50 - 3) . "..." : htmlsafechars($arr["name"])) . "</b></a></div>
...
if (($user['opt1'] & user_options::HIDECUR || $CURUSER['id'] == $id || $CURUSER['class'] >= UC_STAFF) && ($user_snatches_data = $cache->get('user_snatches_data_' . $id)) === false) {
    ...
    ($ressnatch = sql_query("SELECT s.*, t.name AS name, c.name AS catname, c.image AS catimg FROM snatched AS s INNER JOIN torrents AS t ON s.torrentid = t.id LEFT JOIN categories AS c ON t.category = c.id WHERE s.userid =" . sqlesc($user['id'])." AND s.torrentid IN (SELECT id FROM torrents)")) || sqlerr(__FILE__, __LINE__);
    } else {
     ($ressnatch = sql_query("SELECT x.*, t.name AS name, c.name AS catname, c.image AS catimg FROM xbt_peers AS x INNER JOIN torrents AS t ON x.tid = t.id LEFT JOIN categories AS c ON t.category = c.id WHERE x.uid =" . sqlesc($user['id'])." AND x.tid IN (SELECT id FROM torrents)")) || sqlerr(__FILE__, __LINE__);
    }
