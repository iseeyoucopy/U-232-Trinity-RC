...
if ($cache_share_ratio['seed_time_total'] > 0 && $cache_share_ratio['total_number'] > 0) {
    ...
    if ($user["id"] == $CURUSER["id"] || $CURUSER['class'] >= UC_STAFF) {
        $HTMLOUT .= '<tr><td class="clearalt5" align="right"><b>'.$lang['userdetails_time_ratio'].'</b></td><td align="left" class="clearalt5">'.(($user_stats['downloaded'] > 0 || $user_stats['uploaded'] > 2_147_483_648) ? '<font color="'.get_ratio_color(number_format($avg_time_ratio,
                    3)).'">'.number_format($avg_time_ratio, 3).'</font>     '.ratio_image_machine(number_format($avg_time_ratio,
                    3)).'     [<font color="'.get_ratio_color(number_format($avg_time_ratio,
                    3)).'"> '.$avg_time_seeding.'</font>'.$lang['userdetails_time_ratio_per'].'' : $lang['userdetails_inf']).'</td></tr>';
    }
}
//==end
// End Class
