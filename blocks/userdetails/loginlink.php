...
if ($CURUSER['class'] >= UC_STAFF && $id == $CURUSER['id']) {
    $hash1 = $cache->get($cache_keys['hash1'].$id);
    if ($hash1 === false) {
        ...
        $cache->set($cache_keys['hash1'].$id, $hash1, $TRINITY20['expires']['user_hash']);
    }
    $arr = $hash1;
    if ($arr['hash1'] != '') {
        $HTMLOUT .= "<tr><td class='rowhead'>{$lang['userdetails_login_link']}<br /><a href='createlink.php?action=reset&amp;id=".(int)$CURUSER['id']."' target='_blank'>{$lang['userdetails_login_reset']}</a></td><td align='left'>{$TRINITY20['baseurl']}/pagelogin.php?qlogin=".htmlspecialchars($arr['hash1'])."</td></tr>";
    } else {
