...
if (($top10music_week = $cache->get($cache_keys['top10_music_week'])) === false) {
    $tortimeweekmusic = $_SERVER['REQUEST_TIME'] - 604800;
    ($res_musicw = sql_query("SELECT id, times_completed, seeders, poster, leechers, name, category from torrents WHERE last_action >= {$tortimeweekmusic}  AND category IN (".implode(", ",
    ...
    $cache->set($cache_keys['top10_music_week'], $top10music_week);
}
...
if ($top10music_week) {
    ...
    foreach ($top10music_week as $top10music_w_arr) {
        if (is_array($top10music_w_arr)) {
            $top10music_w_arr['cat_name'] = htmlspecialchars($change[$top10music_w_arr['category']]['name']);
            $top10music_w_arr['cat_pic'] = htmlspecialchars($change[$top10music_w_arr['category']]['image']);
            $torrname = htmlspecialchars($top10music_w_arr['name']);
