...
if (($last24_cache = $cache->get($keys['last24'])) === false) {
    ...
    ($res = sql_query('SELECT id, username, class, donor, title, warned, enabled, chatpost, leechwarn, pirate, king, perms '.'FROM users WHERE last_access >= '.$time24.' '.'AND perms < '.bt_options::PERMS_STEALTH.' ORDER BY username ASC')) || sqlerr(__FILE__,
        __LINE__);
    ...
    if ($totalonline24 > $last24) {
        ...
        sql_query('UPDATE avps SET value_s = 0, '.'value_i = '.sqlesc($last24).', '.'value_u = '.sqlesc($period).' '.'WHERE arg = "last24"') || sqlerr(__FILE__,
            __LINE__);
    }
