...
if (($active_users_cache = $cache->get($keys['act_users'])) === false) {
    ...
    $activeusers = '';
    $active_users_cache = [];
    ($res = sql_query('SELECT id, username, class, donor, title, warned, enabled, chatpost, leechwarn, pirate, king, perms '.'FROM users WHERE last_access >= '.$dt.' '.'AND perms < '.bt_options::PERMS_STEALTH.' ORDER BY username ASC')) || sqlerr(__FILE__,
        __LINE__);
