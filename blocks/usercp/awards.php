...
if ($TRINITY20['achieve_sys_on'] == false) {
    ...
    $perpage = 5;
    if (!$count) {
        $HTMLOUT .= "{$lang['achievement_history_err2']}<a href='userdetails.php?id=".(int)$CURUSER['id']."'>".htmlspecialchars($CURUSER['username'])."</a>{$lang['achievement_history_err3']}";
    }
...
<p>{$lang['achievement_history_c']}".htmlspecialchars($row['0'])."{$lang['achievement_history_a']}".($row[0] == 1 ? "" : "s")."</p>
    <p>".htmlspecialchars($achpoints)."{$lang['achievement_history_pa']}".htmlspecialchars($spentpoints)."{$lang['achievement_history_ps']}</p>";
...
<div class='media-object callout'>
  <div class='media-object-section'>
    <div class='thumbnail'>
    <img src='pic/achievements/".htmlspecialchars($arr['icon'])."' alt='".htmlspecialchars($arr['achievement'])."' title='".htmlspecialchars($arr['achievement'])."'>
    </div>
  </div>
  <div class='media-object-section'>
    <p>".htmlspecialchars($arr['description'])."</p>
