...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($_POST["userid"]) && (($action == "deluser") || ($action == "mail"))) stderr($lang['inactive_error'], "{$lang['inactive_selectuser']}");
      if ($action == "deluser" && (!empty($_POST["userid"]))) {
          ...
          stderr($lang['inactive_success'], "{$lang['inactive_deleted']} <a href='" . $TRINITY20['baseurl'] . "/staffpanel.php?tool=inactive>{$lang['inactive_back']}</a>");
      }
    if ($action == "disable" && (!empty($_POST["userid"]))) {
        ...
        stderr($lang['inactive_success'], "{$lang['inactive_disabled']} <a href='" . $TRINITY20['baseurl'] . "/staffpanel.php?tool=inactive>{$lang['inactive_back']}</a>");
    }
