...
// config
$replyto = "{$TRINITY20['site_email']}"; // The Reply-to email.
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($action == "deluser" && (!empty($_POST["userid"]))) {
        $res = sql_query("SELECT id, email, modcomment, username, added, last_access FROM users WHERE id IN (".implode(", ",
                array_map("sqlesc", $_POST['userid'])).") ORDER BY last_access DESC ");
        $count = $res->num_rows;
        ...
        stderr($lang['inactive_success'],
            "{$lang['inactive_deleted']} <a href='".$TRINITY20['baseurl']."/staffpanel.php?tool=inactive>{$lang['inactive_back']}</a>");
    }
    if ($action == "disable" && (!empty($_POST["userid"]))) {
        ...
        stderr($lang['inactive_success'],
            "{$lang['inactive_disabled']} <a href='".$TRINITY20['baseurl']."/staffpanel.php?tool=inactive>{$lang['inactive_back']}</a>");
    }
    if ($action == "mail" && (!empty($_POST["userid"]))) {
        ...
        while ($arr = $res->fetch_array(MYSQLI_BOTH)) {
            ...
            {$lang['inactive_youracc']} {$TRINITY20['site_name']} {$lang['inactive_marked']} {$TRINITY20['site_name']}{$lang['inactive_plogin']}\n
            {$lang['inactive_yourusername']} $username\n
            {$lang['inactive_created']} $added\n
            {$lang['inactive_lastaccess']} $last_access\n
            {$lang['inactive_loginat']} {$TRINITY20['baseurl']}/login.php\n
            {$lang['inactive_forgotten']} {$TRINITY20['baseurl']}/resetpw.php\n
            {$lang['inactive_welcomeback']} {$TRINITY20['site_name']}";
...
($res = sql_query("SELECT id,username,class,email,uploaded,downloaded,last_access FROM users WHERE last_access<".sqlesc($dt)." AND status='confirmed' AND enabled='yes' ORDER BY last_access DESC {$pager['limit']}")) || sqlerr(__FILE__,
    __LINE__);
