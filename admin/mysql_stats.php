...
function localisedDate($timestamp = -1, $format = '')
{
    ...
    $date = preg_replace('@%[aA]@', $day_of_week[(int)date('d M Y', $timestamp)], $format);
    $date = preg_replace('@%[bB]@', $month[(int)date('d M Y', $timestamp) - 1], $date);
...
$TRAFFIC_STATS .= "<table class='table table-bordered'>
            ...
            <tr>
                ...
                <td bgcolor='grey' align='right'>&nbsp;".implode(' ',
        byteformat(($serverStatus['Bytes_received'] + $serverStatus['Bytes_sent']) * 3600 / $serverStatus['Uptime']))."&nbsp;</td>
            </tr>
        </table>";
$TRAFFIC_STATS2 = "<table class='table table-bordered'>
        ...
        <tr>
            ...
            <td  align='right'>&nbsp;".(($serverStatus['Connections'] > 0) ? number_format(($serverStatus['Aborted_connects'] * 100 / $serverStatus['Connections']),
            2, '.', ',')."&nbsp;%" : '---&nbsp;')."</td>
        </tr>
        <tr>
            ...
            <td  align='right'>&nbsp;".(($serverStatus['Connections'] > 0) ? number_format(($serverStatus['Aborted_clients'] * 100 / $serverStatus['Connections']),
            2, '.', ',').'&nbsp;%' : '---')."&nbsp;</td>
        </tr>
        <tr>
            <td bgcolor='grey'>{$lang['mysql_stats_total']}</td>
