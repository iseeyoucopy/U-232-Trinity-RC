...
if (isset($_POST["nowarned"]) && $_POST["nowarned"] == "nowarned") {
    ...
    if (!empty($_POST["desact"])) {
        sql_query("UPDATE users SET enabled = 'no' WHERE id IN (" . implode(", ", array_map("sqlesc", $_POST["desact"])) . ")") || sqlerr(__FILE__, __LINE__);
    }
}
$HTMLOUT.= "<div class='row'><div class='col-md-12'>";
$HTMLOUT.= "<h2>{$lang['cheaters_users']}</h2>";
($res = sql_query("SELECT COUNT(*) FROM cheaters")) || sqlerr(__FILE__, __LINE__);
...
{var klappText=document.getElementById('k'+id);var klappBild=document.getElementById('pic'+id);if(klappText.style.display=='none'){klappText.style.display='block';klappBild.src='{$TRINITY20['pic_base_url']}minus.gif';}
else{klappText.style.display='none';klappBild.src='{$TRINITY20['pic_base_url']}plus.gif';}}	
</script>
<script type='text/javascript'>
var checkflag = 'false';
