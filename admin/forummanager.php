...
switch ($do) {
    ...
    default :
        ...
        ($r1 = sql_query('select f.name as f_name, f.id as fid, f.description,f.min_class_read,f.min_class_write, f.min_class_create, o.name as o_name,o.id as oid FROM forums as f LEFT JOIN over_forums as o ON f.forum_id = o.id ORDER BY f.sort')) || sqlerr(__FILE__,
            __LINE__);
        $f_count = $r1->num_rows;
        if (!$f_count) {
        ...
        if ($do == 'edit' && $id) {
            ($r3 = sql_query('select f.name as f_name , f.id as fid , f.description , f.min_class_read , f.min_class_write , f.min_class_create, f.forum_id, f.sort FROM forums as f WHERE f.id ='.sqlesc($id))) || sqlerr(__FILE__,
                __LINE__);
            if (!$r3->num_rows) {
