...
switch ($action) {
    case 'upload_credit':
        ...
        if ($free_for_classes === 1) {
            ($res_GB = sql_query('SELECT id, uploaded, modcomment FROM users WHERE enabled = \'yes\' AND suspended = \'no\'')) || sqlerr(__FILE__,
                __LINE__);
            ...
            if ($res_GB->num_rows > 0) {
                ...
                while ($arr_GB = $res_GB->fetch_assoc()) {
                    ...
                    $cache->delete('inbox_new::'.$arr_GB['id']);
                    $cache->delete('inbox_new_sb::'.$arr_GB['id']);
                }
                $count = count($users_buffer);
                if ($count > 0) {
                    sql_query("INSERT INTO messages (sender,receiver,added,msg,subject) VALUES ".implode(', ', $pm_buffer)) || sqlerr(__FILE__,
                        __LINE__);
                    sql_query("INSERT INTO users (id, uploaded, modcomment) VALUES ".implode(', ',
                            $users_buffer)." ON DUPLICATE key UPDATE uploaded=values(uploaded),modcomment=values(modcomment)") || sqlerr(__FILE__,
                        __LINE__);
            ...
            foreach ($free_for as $class) {
                if (ctype_digit($class)) {
                    ...
                    if ($res_GB->num_rows > 0) {
                        ...
                        $count = count($users_buffer);
                        if ($count > 0) {
                            sql_query("INSERT INTO messages (sender,receiver,added,msg,subject) VALUES ".implode(', ',
                                    $pm_buffer)) || sqlerr(__FILE__, __LINE__);
                            sql_query("INSERT INTO users (id, uploaded, modcomment) VALUES ".implode(', ',
                                    $users_buffer)." ON DUPLICATE key UPDATE uploaded=values(uploaded),modcomment=values(modcomment)") || sqlerr(__FILE__,
                                __LINE__);
    ...
    case 'karma':
        ...
        //=== if for all classes
        if ($free_for_classes === 1) {
            ($res_karma = sql_query('SELECT id, seedbonus, modcomment FROM users WHERE enabled = \'yes\' AND suspended = \'no\'')) || sqlerr(__FILE__,
                __LINE__);
            $pm_buffer = $users_buffer = [];
            if ($res_karma->num_rows > 0) {
                ...
                while ($arr_karma = $res_karma->fetch_assoc()) {
                    ...
                    $cache->delete('inbox_new::'.$arr_karma['id']);
                    $cache->delete('inbox_new_sb::'.$arr_karma['id']);
                }
                $count = count($users_buffer);
                if ($count > 0) {
                    sql_query("INSERT INTO messages (sender,receiver,added,msg,subject) VALUES ".implode(', ', $pm_buffer)) || sqlerr(__FILE__,
                        __LINE__);
                    sql_query("INSERT INTO users (id, seedbonus, modcomment) VALUES ".implode(', ',
                            $users_buffer)." ON DUPLICATE key UPDATE seedbonus=values(seedbonus),modcomment=values(modcomment)") || sqlerr(__FILE__,
                        __LINE__);
    ...
    case 'freeslots':
        ...
        //=== if for all classes
        if ($free_for_classes === 1) {
            ($res_freeslots = sql_query('SELECT id, freeslots, modcomment FROM users WHERE enabled = \'yes\' AND suspended = \'no\'')) || sqlerr(__FILE__,
                __LINE__);
            $pm_buffer = $users_buffer = [];
            if ($res_freeslots->num_rows > 0) {
                ...
                while ($arr_freeslots = $res_freeslots->fetch_assoc()) {
                    ...
                    $cache->delete('inbox_new::'.$arr_freeslots['id']);
                    $cache->delete('inbox_new_sb::'.$arr_freeslots['id']);
                }
                $count = count($users_buffer);
                if ($count > 0) {
                    sql_query("INSERT INTO messages (sender,receiver,added,msg,subject) VALUES ".implode(', ', $pm_buffer)) || sqlerr(__FILE__,
                        __LINE__);
                    sql_query("INSERT INTO users (id, freeslots, modcomment) VALUES ".implode(', ',
                            $users_buffer)." ON DUPLICATE key UPDATE freeslots=values(freeslots),modcomment=values(modcomment)") || sqlerr(__FILE__,
                        __LINE__);
        ...
        if ($free_for_classes === 1) {
            ...
            if ($res_invites->num_rows > 0) {
                ...
                while ($arr_invites = $res_invites->fetch_assoc()) {
                    ...
                    $cache->delete('inbox_new::'.$arr_invites['id']);
                    $cache->delete('inbox_new_sb::'.$arr_invites['id']);
                }
                $count = count($users_buffer);
                if ($count > 0) {
                    sql_query("INSERT INTO messages (sender,receiver,added,msg,subject) VALUES ".implode(', ', $pm_buffer)) || sqlerr(__FILE__,
                    ...
                    sql_query("INSERT INTO users (id, invites, modcomment) VALUES ".implode(', ',
                            $users_buffer)." ON DUPLICATE key UPDATE invites=values(invites),modcomment=values(modcomment)") || sqlerr(__FILE__,
                        __LINE__);
