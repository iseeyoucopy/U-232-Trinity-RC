...
if (isset($free) && ((is_countable($free) ? count($free) : 0) < 1)) {
    ...
    foreach ($free as $fl) {
        ...
        $HTMLOUT .= "<tr><td>$mode
		     </td><td align='left'>".($fl['expires'] != 'Inf.' && $fl['expires'] != 1 ? "{$lang['freelech_until']}".get_date($fl['expires'],
                    'DATE')." (".mkprettytime($fl['expires'] - TIME_NOW)."{$lang['freelech_togo']})" : ''.$lang['freelech_unlimited'].'')." </td>
