...
function remove_torrent_pass($torrent_pass)
{
    ...
    $key = $cache_keys['user_torrent_pass'].$torrent_pass;
    $cache->delete($key);
}
...
if ((isset($_POST['action'])) && ($_POST['action'] == "edituser")) {
    ...
    $setbits = $clrbits = 0;
    $username = (($CURUSER['perms'] & bt_options::PERMS_STEALTH) !== 0 ? 'System' : htmlspecialchars($CURUSER['username']));
    ...
    if ((isset($_POST['class'])) && (($class = $_POST['class']) != $user['class'])) {
        ...
        sql_query("INSERT INTO messages (sender, receiver, msg, subject, added) VALUES(0, ".sqlesc($userid).", $msg, $subject, $added)") || sqlerr(__FILE__,
            __LINE__);
        ...
        $modcomment = get_date(TIME_NOW, 'DATE',
                1)." - $what {$lang['modtask_to']} '".get_user_class_name($class)."'{$lang['modtask_gl_by']} {$CURUSER['username']}.\n".$modcomment;
    }
    ...
    if ((isset($_POST['donorlength'])) && ($donorlength = 0 + $_POST['donorlength'])) {
        ...
        $added = sqlesc(TIME_NOW);
        sql_query("INSERT INTO messages (sender, subject, receiver, msg, added) VALUES (0, $subject, ".sqlesc($userid).", $msg, $added)") || sqlerr(__file__,
            __line__);
        $updateset[] = "donor = 'yes'";
    ...
    if ((isset($_POST['donorlengthadd'])) && ($donorlengthadd = 0 + $_POST['donorlengthadd'])) {
        ...
        $msg = sqlesc($lang['modtask_donor_dear'].htmlspecialchars($user['username'])."{$lang['modtask_donor_msg2']} $dur {$lang['modtask_donor_msg3']}".$username);
        $subject = sqlesc($lang['modtask_donor_subject_again']);
        $modcomment = get_date(TIME_NOW, 'DATE',
                1)."{$lang['modtask_donor_set_another']} $dur {$lang['modtask_gl_by']}".$CURUSER['username'].".\n".$modcomment;
        $donorlengthadd *= 7;
        sql_query("UPDATE users SET vipclass_before=".sqlesc($user["class"]).", donoruntil = IF(donoruntil=0, ".TIME_NOW." + 86400 * $donorlengthadd, donoruntil + 86400 * $donorlengthadd) WHERE id =".sqlesc($userid)) || sqlerr(__file__,
            __line__);
        $added = TIME_NOW;
        sql_query("INSERT INTO messages (sender, subject, receiver, msg, added) VALUES (0, $subject, ".sqlesc($userid).", $msg, $added)") || sqlerr(__file__,
            __line__);
    ...
    if (isset($_POST['downloadpos']) && ($downloadpos = 0 + $_POST['downloadpos'])) {
        ...
        $added = sqlesc(TIME_NOW);
        if ($downloadpos == 255) {
            $modcomment = get_date($added, 'DATE',
                    1).$lang['modtask_down_dis_by'].$CURUSER['username'].".\n{$lang['modtask_gl_reason']} $disable_pm\n".$modcomment;
            ...
            $modcomment = get_date($added, 'DATE',
                    1)."{$lang['modtask_down_dis_for']} $dur {$lang['modtask_gl_by']}".$CURUSER['username'].".\n{$lang['modtask_gl_reason']} $disable_pm\n".$modcomment;
    ...
    if (isset($_POST['uploadpos']) && ($uploadpos = 0 + $_POST['uploadpos'])) {
        ...
        if ($uploadpos == 255) {
            $modcomment = get_date($added, 'DATE',
                    1).$lang['modtask_up_dis_by'].$CURUSER['username'].".\n{$lang['modtask_gl_reason']} $updisable_pm\n".$modcomment;
            ...
            $modcomment = get_date($added, 'DATE',
                    1)."{$lang['modtask_up_dis_for']} $dur {$lang['modtask_gl_by']}".$CURUSER['username'].".\n{$lang['modtask_gl_reason']} $updisable_pm\n".$modcomment;
    ...
    if (isset($_POST['sendpmpos']) && ($sendpmpos = 0 + $_POST['sendpmpos'])) {
        ...
        if ($sendpmpos == 255) {
            $modcomment = get_date($added, 'DATE',
                    1).$lang['modtask_pm_dis_by'].$CURUSER['username'].".\n{$lang['modtask_gl_reason']} $pmdisable_pm\n".$modcomment;
            ...
            $modcomment = get_date($added, 'DATE',
                    1)."{$lang['modtask_pm_dis_for']} $dur {$lang['modtask_gl_by']}".$CURUSER['username'].".\n{$lang['modtask_gl_reason']} $pmdisable_pm\n".$modcomment;
    ...
    if (isset($_POST['chatpost']) && ($chatpost = 0 + $_POST['chatpost'])) {
        ...
        if ($chatpost == 255) {
            $modcomment = get_date($added, 'DATE',
                    1).$lang['modtask_shout_dis_by'].$CURUSER['username'].".\n{$lang['modtask_gl_reason']} $chatdisable_pm\n".$modcomment;
            ...
            $modcomment = get_date($added, 'DATE',
                    1)."{$lang['modtask_shout_dis_for']} $dur {$lang['modtask_gl_by']}".$CURUSER['username'].".\n{$lang['modtask_gl_reason']} $chatdisable_pm\n".$modcomment;
    ...
    if (isset($_POST['immunity']) && ($immunity = 0 + $_POST['immunity'])) {
        ...
        if ($immunity == 255) {
            $modcomment = get_date($added, 'DATE',
                    1).$lang['modtask_immune_status'].$CURUSER['username'].".\n{$lang['modtask_gl_reason']} $immunity_pm\n".$modcomment;
            ...
            $modcomment = get_date($added, 'DATE',
                    1)."{$lang['modtask_immune_status_for']} $dur {$lang['modtask_gl_by']}".$CURUSER['username'].".\n{$lang['modtask_gl_reason']} $immunity_pm\n".$modcomment;
    ...
    if (isset($_POST['leechwarn']) && ($leechwarn = 0 + $_POST['leechwarn'])) {
        ...
        if ($leechwarn == 255) {
            $modcomment = get_date($added, 'DATE',
                    1).$lang['modtask_leechwarn_status'].$CURUSER['username'].".\n{$lang['modtask_gl_reason']} $leechwarn_pm\n".$modcomment;
            ...
            $modcomment = get_date($added, 'DATE',
                    1)."{$lang['modtask_leechwarn_status_for']} $dur {$lang['modtask_gl_by']}".$CURUSER['username'].".\n{$lang['modtask_gl_reason']} $leechwarn_pm\n".$modcomment;
    ...
    if (isset($_POST['warned']) && ($warned = 0 + $_POST['warned'])) {
        ...
        if ($warned == 255) {
            $modcomment = get_date($added, 'DATE',
                    1).$lang['modtask_warned_status'].$CURUSER['username'].".\n{$lang['modtask_gl_reason']} $warned_pm\n".$modcomment;
            ...
            $modcomment = get_date($added, 'DATE',
                    1)."{$lang['modtask_warned_status_for']} $dur {$lang['modtask_gl_by']}".$CURUSER['username'].".\n{$lang['modtask_gl_reason']} $warned_pm\n".$modcomment;
    ...
    if ($CURUSER['class'] >= UC_ADMINISTRATOR) {
        ...
        if ($uploadtoadd > 0) {
            if ($mpup == "plus") {
                ...
                $modcomment = get_date(TIME_NOW, 'DATE',
                        1)." {$lang['modtask_add_upload']} (".$uploadtoadd." ".$formatup.") {$lang['modtask_by']} ".$CURUSER['username']."\n".$modcomment;
            } else {
                $newupload = $user["uploaded"] - ($formatup == 'mb' ? ($uploadtoadd * 1_048_576) : ($uploadtoadd * 1_073_741_824));
                if ($newupload >= 0) {
                    $modcomment = get_date(TIME_NOW, 'DATE',
                            1)." {$lang['modtask_subtract_upload']} (".$uploadtoadd." ".$formatup.") {$lang['modtask_by']} ".$CURUSER['username']."\n".$modcomment;
                }
            }
            if ($newupload >= 0) {
        ...
        if ($downloadtoadd > 0) {
            if ($mpdown == "plus") {
                ...
                $modcomment = get_date(TIME_NOW, 'DATE',
                        1)." {$lang['modtask_added_download']} (".$downloadtoadd." ".$formatdown.") {$lang['modtask_by']} ".$CURUSER['username']."\n".$modcomment;
            } else {
                $newdownload = $user["downloaded"] - ($formatdown == 'mb' ? ($downloadtoadd * 1_048_576) : ($downloadtoadd * 1_073_741_824));
                if ($newdownload >= 0) {
                    $modcomment = get_date(TIME_NOW, 'DATE',
                            1)." {$lang['modtask_subtract_download']} (".$downloadtoadd." ".$formatdown.") {$lang['modtask_by']} ".$CURUSER['username']."\n".$modcomment;
                }
            }
            if ($newdownload >= 0) {
    ...
    if ((isset($_POST['title'])) && (($title = $_POST['title']) != ($curtitle = $user['title']))) {
        $modcomment = get_date(TIME_NOW, 'DATE',
                1)."{$lang['modtask_custom_title']}'".$title."'{$lang['modtask_gl_from']}'".$curtitle."'{$lang['modtask_by']}".$CURUSER['username'].".\n".$modcomment;
    ...
    if ((isset($_POST['reset_torrent_pass'])) && ($_POST['reset_torrent_pass'])) {
        ...
        $modcomment = get_date(TIME_NOW, 'DATE',
                1)."{$lang['modtask_passkey']}".sqlesc($user['torrent_pass'])."{$lang['modtask_reset']}".sqlesc($newpasskey)."{$lang['modtask_by']}".$CURUSER['username'].".\n".$modcomment;
    ...
    if ((isset($_POST['seedbonus'])) && (($seedbonus = $_POST['seedbonus']) != ($curseedbonus = $user['seedbonus']))) {
        $modcomment = get_date(TIME_NOW, 'DATE',
                1).$lang['modtask_seedbonus'].$seedbonus.$lang['modtask_gl_from'].$curseedbonus.$lang['modtask_gl_by'].$CURUSER['username'].".\n".$modcomment;
    ...
    if ((isset($_POST['reputation'])) && (($reputation = $_POST['reputation']) != ($curreputation = $user['reputation']))) {
        $modcomment = get_date(TIME_NOW, 'DATE',
                1).$lang['modtask_reputation'].$reputation.$lang['modtask_gl_from'].$curreputation.$lang['modtask_gl_by'].$CURUSER['username'].".\n".$modcomment;
    ...
    if ((isset($_POST['avatar'])) && (($avatar = $_POST['avatar']) != ($curavatar = $user['avatar']))) {
        ...
        $modcomment = get_date(TIME_NOW, 'DATE',
                1)."{$lang['modtask_avatar_change']}".htmlspecialchars($curavatar)."{$lang['modtask_to']}".htmlspecialchars($avatar)."{$lang['modtask_by']} ".$CURUSER['username'].".\n".$modcomment;
    ...
    if ((isset($_POST['signature'])) && (($signature = $_POST['signature']) != ($cursignature = $user['signature']))) {
        ...
        $modcomment = get_date(TIME_NOW, 'DATE',
                1)."{$lang['modtask_signature_change']}".htmlspecialchars($cursignature)."{$lang['modtask_to']}".htmlspecialchars($signature)."{$lang['modtask_by']} ".$CURUSER['username'].".\n".$modcomment;
    ...
    if ((isset($_POST['invite_on'])) && (($invite_on = $_POST['invite_on']) != $user['invite_on'])) {
        $modcomment = get_date(TIME_NOW, 'DATE',
                1).$lang['modtask_invites_allowed'].htmlspecialchars($user['invite_on'])."{$lang['modtask_to']}$invite_on{$lang['modtask_gl_by']}".$CURUSER['username'].".\n".$modcomment;
        $updateset[] = "invite_on = ".sqlesc($invite_on);
        $useredit['update'][] = $lang['modtask_invites_enabled'].htmlspecialchars($invite_on).'';
    ...
    if ((isset($_POST['invites'])) && (($invites = $_POST['invites']) != ($curinvites = $user['invites']))) {
        $modcomment = get_date(TIME_NOW, 'DATE',
                1).$lang['modtask_invites_amount'].$invites.$lang['modtask_gl_from'].$curinvites.$lang['modtask_gl_by'].$CURUSER['username'].".\n".$modcomment;
    ...
    if ((isset($_POST['freeslots'])) && (($freeslots = $_POST['freeslots']) != ($curfreeslots = $user['freeslots']))) {
        $modcomment = get_date(TIME_NOW, 'DATE',
                1).$lang['modtask_freeslots_amount'].$freeslots.$lang['modtask_gl_from'].$curfreeslots.$lang['modtask_gl_by'].$CURUSER['username'].".\n".$modcomment;
    ...
    if (isset($_POST['free_switch']) && ($free_switch = 0 + $_POST['free_switch'])) {
        ...
        if ($free_switch == 255) {
            $modcomment = get_date($added, 'DATE',
                    1).$lang['modtask_freeleech_status'].$CURUSER['username'].".\n{$lang['modtask_gl_reason']} $free_pm\n".$modcomment;
            ...
            $modcomment = get_date($added, 'DATE',
                    1)."{$lang['modtask_freeleech_for']} $dur {$lang['modtask_gl_by']}".$CURUSER['username'].".\n{$lang['modtask_gl_reason']} $free_pm\n".$modcomment;
    ...
    if (isset($_POST['game_access']) && ($game_access = 0 + $_POST['game_access'])) {
        ...
        if (isset($_POST['game_disable_pm'])) {
            $game_disable_pm = $_POST['game_disable_pm'];
        }
        $subject = sqlesc($lang['modtask_gl_notification']);
        $added = sqlesc(TIME_NOW);
        if ($game_access == 255) {
            $modcomment = get_date($added, 'DATE',
                    1).$lang['modtask_games_dis_by'].$CURUSER['username'].".\n{$lang['modtask_gl_reason']} $game_disable_pm\n".$modcomment;
            ...
            $modcomment = get_date($added, 'DATE',
                    1)."{$lang['modtask_games_dis_for']} $dur {$lang['modtask_gl_by']}".$CURUSER['username'].".\n{$lang['modtask_gl_reason']} $game_disable_pm\n".$modcomment;
    ...
    if (isset($_POST['avatarpos']) && ($avatarpos = 0 + $_POST['avatarpos'])) {
        ...
        if ($avatarpos == 255) {
            $modcomment = get_date($added, 'DATE',
                    1).$lang['modtask_avatar_dis_by'].$CURUSER['username'].".\n{$lang['modtask_gl_reason']} $avatardisable_pm\n".$modcomment;
            ...
            $modcomment = get_date($added, 'DATE',
                    1)."{$lang['modtask_avatar_dis_for']} $dur {$lang['modtask_gl_by']}".$CURUSER['username'].".\n{$lang['modtask_gl_reason']} $avatardisable_pm\n".$modcomment;
    ...
    if ((isset($_POST['wait_time'])) && (($wait_time = $_POST['wait_time']) != $user['wait_time'])) {
        $modcomment = get_date(TIME_NOW, 'DATE',
                1)."{$lang['modtask_wait_set']} $wait_time{$lang['modtask_gl_was']}".(int)$user['wait_time'].$lang['modtask_gl_by'].$CURUSER['username'].".\n".$modcomment;
    ...
    if ((isset($_POST['peers_limit'])) && (($peers_limit = $_POST['peers_limit']) != $user['peers_limit'])) {
        $modcomment = get_date(TIME_NOW, 'DATE',
                1)."{$lang['modtask_peer_limit']} $peers_limit{$lang['modtask_gl_was']}".(int)$user['peers_limit'].$lang['modtask_gl_by'].$CURUSER['username'].".\n".$modcomment;
    ...
    if ((isset($_POST['torrents_limit'])) && (($torrents_limit = $_POST['torrents_limit']) != $user['torrents_limit'])) {
        $modcomment = get_date(TIME_NOW, 'DATE',
                1)."{$lang['modtask_torrent_limit']} $torrents_limit{$lang['modtask_gl_was']}".(int)$user['torrents_limit'].$lang['modtask_gl_by'].$CURUSER['username'].".\n".$modcomment;
    ...
    if ((isset($_POST['suspended'])) && (($suspended = $_POST['suspended']) != ($suspended = $user['suspended']))) {
        ...
        if ($_POST['suspended'] === 'yes') {
            $modcomment = get_date(TIME_NOW, 'DATE',
                    1).$lang['modtask_suspend_by'].$CURUSER['username'].$lang['modtask_suspend_reason'].sqlesc($suspended_reason).".\n".$modcomment;
            $updateset[] = "downloadpos = '0'";
            $updateset[] = "uploadpos = '0'";
            ...
            //=== post to forum
            $body = sqlesc("{$lang['modtask_suspend_acc_for']}[b][url=".$TRINITY20['baseurl']."/userdetails.php?id=".(int)$user["id"]."]".htmlspecialchars($user["username"])."[/url][/b]{$lang['modtask_suspend_has_by']}".$CURUSER['username']."\n\n [b]{$lang['modtask_suspend_reason']}[/b]\n ".sqlesc($suspended_reason).".\n");
            auto_post($subject, $body);
        }
        if ($_POST['suspended'] === 'no') {
            $modcomment = get_date(TIME_NOW, 'DATE',
                    1).$lang['modtask_unsuspend_by'].$CURUSER['username'].$lang['modtask_suspend_reason'].sqlesc($suspended_reason).".\n".$modcomment;
            $updateset[] = "downloadpos = '1'";
            $updateset[] = "uploadpos = '1'";
    ...
    if ((isset($_POST['hit_and_run_total'])) && (($hit_and_run_total = $_POST['hit_and_run_total']) != $user['hit_and_run_total'])) {
        $modcomment = get_date(TIME_NOW, 'DATE',
                1)."{$lang['modtask_hit_run_set']} $hit_and_run_total{$lang['modtask_gl_was']}".(int)$user['hit_and_run_total'].$lang['modtask_gl_by'].$CURUSER['username'].".\n".$modcomment;
    ...
    if ((isset($_POST['website'])) && (($website = $_POST['website']) != $user['website'])) {
        $modcomment = get_date(TIME_NOW, 'DATE',
                1).$lang['modtask_website_changed_to'].strip_tags($_POST['website']).$lang['modtask_gl_from'].htmlspecialchars($user['website']).$lang['modtask_gl_by'].$CURUSER['username'].".\n".$modcomment;
    ...
    if (isset($_POST["forum_mod"]) && ($forum_mod = $_POST["forum_mod"]) != $user["forum_mod"]) {
        ...
        $cache->delete($cache_keys['forummods']);
    ...
    if ($curuser_cache) {
        $cache->update_row($cache_keys['my_userid'].$userid, $curuser_cache, $TRINITY20['expires']['curuser']);
    }
    if ($user_cache) {
        $cache->update_row($cache_keys['user'].$userid, $user_cache, $TRINITY20['expires']['user_cache']);
    }
    if ($stats_cache !== []) {
        $cache->update_row($cache_keys['user_stats'].$userid, $stats_cache, $TRINITY20['expires']['u_stats']);
    }
    if ($user_stats_cache !== []) {
        $cache->update_row($cache_keys['user_statss'].$userid, $user_stats_cache, $TRINITY20['expires']['user_stats']);
    }
    ...
    if (is_array($setbits) && (is_countable($setbits) ? count($setbits) : 0) > 0 || is_array($clrbits) && (is_countable($clrbits) ? count($clrbits) : 0) > 0) {
        sql_query('UPDATE users SET opt1 = ((opt1 | '.$setbits.') & ~'.$clrbits.'), opt2 = ((opt2 | '.$setbits.') & ~'.$clrbits.') WHERE id = '.sqlesc($userid)) || sqlerr(__file__,
            __line__);
    }
    // grab current data
    ...
    write_info("{$lang['modtask_sysop_user_acc']} $userid (<a href='userdetails.php?id=$userid'>".htmlspecialchars($user['username'])."</a>)\n{$lang['modtask_sysop_thing']}".implode(', ',
            $useredit['update'])."{$lang['modtask_gl_by']}<a href='userdetails.php?id={$CURUSER['id']}'>{$CURUSER['username']}</a>");
