...
function do_sort($arr, $empty = false)
{
    ...
    if ($empty) {
        ...
        while ($res = $arr->fetch_assoc()) {
            $ret_html .= "<tr><td align='center'><a href='details.php?id=".(int)$res['id']."'>".htmlspecialchars($res['name'])."</a></td><td align='center'>".date('h:i:s d/m/Y',
                    $res['added'])."</td><td align='center'><a class='btn-small btn btn-danger' href='edit.php?id=".(int)$res['id']."' >".$lang['mtor_edit']."</a></td></tr>";
        }
        return $ret_html;
    }
    if ($count == 1) {
        ...
        $ret_html .= "<tr><td align='center'><a href='details.php?id=".(int)$res['id']."'>".htmlspecialchars($res['name'])."</a></td><td align='center'><a href='userdetails.php?id=".(int)$res['uid']."'><font color='#".get_user_class_color($CURUSER['class'])."'>".htmlspecialchars($res['checked_by'])."</font></a></td><td align='center'>".date('h:i:s d/m/Y',
                $res['checked_when'])."</td></tr>";
        return [
            $users,
            $ret_html,
        ];
    ...
    if ($count > 1) {
        while ($res = $arr->fetch_assoc()) {
            $users[$res['checked_by']] = ((isset($users[$res['checked_by']]) && $users[$res['checked_by']] > 0) ? $users[$res['checked_by']] + 1 : 1);
            $ret_html .= "<tr><td align='center'><a href='details.php?id=".(int)$res['id']."'>".htmlspecialchars($res['name'])."</a></td><td align='center'><a href='userdetails.php?id=".(int)$res['uid']."'>".htmlspecialchars($res['checked_by'])."</a></td><td align='center'>".date('h:i:s d/m/Y',
                    $res['checked_when'])."</td></tr>";
        }
        return [
            $users,
...
if (isset($_GET['type']) && in_array($_GET['type'], $modes)) {
    $mode = (isset($_GET['type']) && in_array($_GET['type'], $modes)) ? $_GET['type'] : stderr($lang['mtor_error'],
        "".$lang['mtor_please_try_that_previous_request_again'].".");
    if ($mode == 'unmodded') {
