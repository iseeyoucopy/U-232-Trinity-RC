...
if (isset($_GET['add'])) {
    ...
    if (is_valid_id($member_whos_been_bad)) {
        ...
        ($res = sql_query('SELECT modcomment, watched_user, watched_user_reason, username FROM users WHERE id='.sqlesc($member_whos_been_bad))) || sqlerr(__FILE__,
            __LINE__);
        $user = $res->fetch_assoc();
...
$HTMLOUT .= '<div class="row"><div class="col-md-12"> '.$H1_thingie.'<br />
    ...
    <table class="table table-bordered">';
//=== get the member info...
($res = sql_query('SELECT id, username, added, watched_user_reason, watched_user, uploaded, downloaded, warned, suspended, enabled, donor, class, leechwarn, chatpost, pirate, king, invitedby FROM users WHERE watched_user != \'0\' ORDER BY '.$ORDER_BY.$ASC)) || sqlerr(__FILE__,
    __LINE__);
...
if ($how_many > 0) {
    ...
    while ($arr = @$res->fetch_assoc()) {
        ...
        ($invitor_res = sql_query('SELECT id, username, donor, class, enabled, warned, leechwarn, chatpost, pirate, king, suspended FROM users WHERE id='.sqlesc($arr['invitedby']))) || sqlerr(__FILE__,
            __LINE__);
