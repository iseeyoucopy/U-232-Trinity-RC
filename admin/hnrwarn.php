...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($act == "disable") {
        ...
        if (count($pms) > 0) {
            ...
            ($q1 = sql_query("UPDATE users set hnrwarn='no', modcomment=CONCAT(".sqlesc(get_date(TIME_NOW, 'DATE',
                        1).$lang['hnrwarn_rem_log'].$CURUSER['username']."\n").",modcomment) WHERE id IN (".implode(",",
                    $_uids).")")) || sqlerr(__FILE__, __LINE__);
            if ($g && $q1) {
                header("Refresh: 2; url=".$r);
