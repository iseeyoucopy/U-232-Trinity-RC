...
function makepoll()
{
    ...
    if (!empty($questions) && count((array) $questions) > $INSTALLER09['max_poll_questions']) {
        exit('poll_to_many');
    }
    if (!empty($choices_count) && count((array) $choices_count) > ($INSTALLER09['max_poll_questions'] * $INSTALLER09['max_poll_choices_per_question'])) {
        exit('poll_to_many');
    }
