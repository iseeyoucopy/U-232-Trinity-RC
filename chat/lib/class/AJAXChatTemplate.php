...
class AJAXChatTemplate
{
    ...
    function getLogsChannelOptionTags()
    {
        ...
        foreach ($this->ajaxChat->getChannels() as $key => $value) {
            if ($this->ajaxChat->getUserRole() >= UC_MODERATOR && $this->ajaxChat->getConfig('logsUserAccessChannelList') && !in_array($value,
                    $this->ajaxChat->getConfig('logsUserAccessChannelList'))) {
                continue;
            }
