...
switch ($textParts[0]) {
    ...
    case '!identify':
        ...
        if ($this->getUserRole() == UC_ADMINISTRATOR && $textParts[1]) {
            ...
            $this->insertChatBotMessage($this->getChannel(),
                "Your username is ".$this->getUserName()." and your user ID is ".$this->getIDFromName($this->getUserName()));
        }
        break;
    case '!flip':
        $winner = null;
        ...
        if ($this->getUserRole() == UC_ADMINISTRATOR) {
            ...
            if (substr($textParts[2], 0, 9) == 'ajax_chat') {
                switch (strtolower($textParts[1])) {
                        ...
                        if ($textParts[2] && $textParts[3] != null && $textParts[4] != null) {
                            ...
                            if ($query == true) {
                                ...
                                $this->insertChatBotMessage($this->getPrivateMessageID(),
                                    "Entry added to database successfully as entry number ".$lastquery);
                            } else {
                                $this->insertChatBotMessage($this->getPrivateMessageID(), "[color=red]Error:[/color] Database error");
                            }
                        } else {
                            $this->insertChatBotMessage($this->getPrivateMessageID(),
                                "[color=red]Error:[/color] You must provide a value to add (and optionally a name)");
                        }
                        break;
                    case 'delete':
                        //	Table				ID
                        if ($textParts[2] && is_numeric($textParts[3])) {
                            ...
                            if ($query && ($this->getIDFromName($this->getUserName()) == $result['user'] || $this->getUserRole() == UC_ADMIN)) {
                                ...
                                $this->insertChatBotMessage($this->getPrivateMessageID(),
                                    "[color=red]Error:[/color] You do not have permission to remove that entry");
                            }
                        } else {
                            $this->insertChatBotMessage($this->getPrivateMessageID(),
                                "[color=red]Error:[/color] You must provide a ID number to remove");
                        }
                        break;
                    case 'show':
                        //	Table				ID num
                        if ($textParts[2] && $textParts[3]) {
                            ...
                            $this->insertChatBotMessage($this->getPrivateMessageID(),
                                "[color=red]Error:[/color] You must provide an ID number to show"
                            );
                        }
                        break;
                    case 'list':
                    ...
                    default:
                        $this->insertChatBotMessage($this->getPrivateMessageID(),
                            "[color=red]Error:[/color] You can only perform add, remove, show and list on the database"
                        );
                        break;
                }
            } else {
                $this->insertChatBotMessage($this->getPrivateMessageID(),
                    "[color=red]Error:[/color] You can only perform operations on AJAX chat tables");
            }
        } else {
    ...
    case '!cmd':
        ...
        switch ($_operation) {
                ...
                if ($_name && $_value) {
                    ...
                    if ($query) {
                        ...
                        if ($rows != 1) {
                            ...
                            if ($query == true) {
                                $this->insertChatBotMessage($this->getChannel(),
                                    "Custom command [b]".$_name."[/b] has been added by ".$this->getUserName());
                            } else {
                                $this->insertChatBotMessage($this->getPrivateMessageID(), "[color=red]Error:[/color] Database error");
                            }
                        } else {
                            ...
                            if ($locked == 0 || $this->getUserID() == $author) {
                                ...
                                if ($this->getUserID() == $author) {
                                    ...
                                    if ($_lock) {
                                        $this->insertChatBotMessage($this->getPrivateMessageID(),
                                            "[color=red]Error:[/color] You cannot lock or unlock a command that is not yours");
                                    }
                                }
                                ...
                                if ($query) {
                                    $this->insertChatBotMessage($this->getChannel(),
                                        "Custom command [b]".$_name."[/b] has been updated by ".$this->getUserName()."".$islocked."");
                                } else {
                                    $this->insertChatBotMessage($this->getPrivateMessageID(), "[color=red]Error:[/color] Database error");
                                }
                            } else {
                                ...
                                $this->insertChatBotMessage($this->getPrivateMessageID(),
                                    "[color=red]Error:[/color] This custom command is locked".$lockedby);
                            }
                        }
                    }
                } else {
            ...
            default:
                $this->insertChatBotMessage($this->getPrivateMessageID(),
                    "[color=red]Error:[/color] Provide a parameter, !cmd ADD NAME VALUE -LOCK/UNLOCK, !cmd LIST -ALL or !cmd LOOKUP");
        }
        break;
        ...
        switch ($textParts[1]) {
                    ...
                    if ($query && $query1 && $query2) {
                        ...
                        if ($trivia['value'] == 'on') {
                            if ($question['value'] == null && $answer['value'] == null) {
                                ...
                                if ($query1 == true && $query2 == true) {
                                    ...
                                    if ($textParts[4] == 'add') {
                                        ...
                                        if ($query3 == true) {
                                            // Tell user
                                            $this->insertChatBotMessage($this->getPrivateMessageID(),
                                                "Question and answer has been added to the trivia database");
                                        } else {
                                            $this->insertChatBotMessage($this->getPrivateMessageID(), "[color=red]Error:[/color] Database error");
                                        }
                                    }
                                ...
                                $this->insertChatBotMessage($this->getPrivateMessageID(),
                                    "[color=red]Error:[/color] Somebody has already asked a question");
                            }
                        } else {
                        ...
                        $this->insertChatBotMessage($this->getPrivateMessageID(),
                            "[color=red]Error:[/color] Database error (check if the question, answer and trivia entries exist in database)");
                    }
                } else {
            ...
            case 'answer':
                ...
                if ($textParts[2] != null) {
                            ...
                            if ($this->getIDFromName($this->getUserName()) != $question['user']) {
                                ...
                                if ($answer['value'] != null) {
                                    ...
                                    if (strtolower($textParts[2]) == strtolower($answer['value'])) {
                                        ...
                                        // Success!
                                        $this->insertChatBotMessage($this->getChannel(),
                                            "[color=blue]Trivia:[/color] Correct! ".$this->getUserName()." has answered correctly with \"".$answer['value']."\"");
                                    ...
                                    $this->insertChatBotMessage($this->getPrivateMessageID(),
                                        "[color=red]Error:[/color] No answer to guess (either trivia mode is off or there is no question)");
                                }
                            } else {
                                $this->insertChatBotMessage($this->getPrivateMessageID(),
                                    "[color=red]Error:[/color] You cannot answer your own question");
                            }
                        } else {
                            $this->insertChatBotMessage($this->getPrivateMessageID(),
                                "[color=red]Error:[/color] You cannot answer more than once in a row");
                        }
                    } else {
                        $this->insertChatBotMessage($this->getPrivateMessageID(), "[color=red]Error:[/color] Database error");
                    }
                    ...
                    $this->insertChatBotMessage($this->getPrivateMessageID(),
                        "[color=red]Error:[/color] You must provide an answer with !trivia answer VALUE");
                }
                break;
            case 'speedround':
                // Only admins or mods
                    ...
                    if ($query && $textParts[2]) {
                        ...
                        if ($trivia['value'] == 'on') {
                            if ($textParts[2] == 'on') {
                                ...
                                if ($query == true) {
                                    $this->insertChatBotMessage($this->getChannel(),
                                        "[color=blue]Trivia:[/color] Speed round [color=green]on[/color]");
                                } else {
                                    $this->insertChatBotMessage($this->getPrivateMessageID(), "[color=red]Error:[/color] Database error");
                                }
                            } else {
                                ...
                                if ($query == true) {
                                    $this->insertChatBotMessage($this->getChannel(),
                                        "[color=blue]Trivia:[/color] Speed round [color=red]off[/color]");
                                } else {
                                    $this->insertChatBotMessage($this->getPrivateMessageID(), "[color=red]Error:[/color] Database error");
                                }
                            }
            ...
            case 'status':
                ...
                if ($query && $query1) {
                    ...
                    if ($trivia['value'] == 'on') {
                        $status = 'Trivia mode is [color=green][b]on[/b][/color]';
                    } else {
                        $status .= 'Trivia mode is [color=red][b]off[/b][/color]';
                    }
                    if ($question['value'] == '') {
                        $status .= ', there is no question';
                    } else {
                        $status .= ', question is [i]'.$question['value'].'[/i]';
                    }
            ...
            case 'on':
                ...
                if ($this->getUserRole() == UC_ADMINISTRATOR || $this->getUserRole() == UC_MODERATOR || $this->getIDFromName($this->getUserName()) == 9 || $this->getIDFromName($this->getUserName()) == 24) {
                    ...
                    if ($query) {
                        ...
                        if ($trivia['value'] == 'on') {
                            $this->insertChatBotMessage($this->getPrivateMessageID(),
                                "[color=red]Error:[/color] Trivia has already started, type !trivia OFF to turn it off");
                        } else {
                    ...
                    if ($trivia['value'] != '') {
                        ...
                        if ($newcount >= $votesreq) {
                            ...
                            if ($query1 == true && $query2 == true && $query3 == true && $query4 == true) {
                                $this->insertChatBotMessage($this->getChannel(),
                                    "[color=blue]Trivia:[/color] Required votes to skip has been reached, question skipped!");
                            } else {
                                $this->insertChatBotMessage($this->getPrivateMessageID(), "[color=red]Error:[/color] Database error");
                            }
                        } else {
                            ...
                            $this->insertChatBotMessage($this->getChannel(),
                                $this->getUserName()." has voted to skip the current question (".$newcount." votes, ".$votesreq1." required)");
                        }
                    } else {
                        $this->insertChatBotMessage($this->getPrivateMessageID(), "[color=red]Error:[/color] Database error");
                    }
            ...
            default:
                ...
                if ($query && $query1 && $query2) {
                    ...
                    if ($trivia['value'] == 'on') {
                        if ($question['value'] == null && $answer['value'] == null) {
                            ...
                            if ($query1 == true && $query2 == true) {
                                ...
                                // Public message
                                $this->insertChatBotMessage('Public',
                                    "[color=blue]Trivia:[/color] Question: [i]".$textParts[2]."[/i] [color=silver](".$random['id'].")[/color] ".$addedby);
                    ...
                    $this->insertChatBotMessage($this->getPrivateMessageID(),
                        "[color=red]Error:[/color] Database error, check if the question, answer and trivia entries are there");
                }
                break;
        }
        break;
...
//				break;
                case 'elim':
                    ...
                    if ($this->getUserRole() == UC_ADMINISTRATOR || $this->getUserRole() == UC_MODERATOR) {
                        ...
                        if ($hotpotato['value'] == 'on') {
                            ...
                            $this->insertChatBotMessage($this->getPrivateMessageID(),
                                "[color=red]Error:[/color] You cannot start elimination mode when Hot Potato is off");
                        }
                    } else {
                        $this->insertChatBotMessage($this->getPrivateMessageID(), "[color=red]Error:[/color] You do not have permission");
                    }
                    ...
                    if ($this->getUserRole() == UC_ADMINISTRATOR || $this->getUserRole() == UC_MODERATOR) {
                        ...
                        if ($hotpotato['value'] == 'off') {
                            ...
                            if ($query_on == true && $query_user == true && $query_counter == true) {
                                $this->insertChatBotMessage($this->getChannel(),
                                    "[color=orange]Hot Potato:[/color] Hot potato is now [color=green]on[/color]");
                            } else {
                                $this->insertChatBotMessage($this->getPrivateMessageID(), "[color=red]Error:[/color] Database error");
                            }
                        } else {
                            $this->insertChatBotMessage($this->getPrivateMessageID(),
                                "[color=red]Error:[/color] Hot potato has already started, type !hot OFF to turn it off");
                        }
                    } else {
                        $this->insertChatBotMessage($this->getPrivateMessageID(), "[color=red]Error:[/color] You do not have permission");
                    }
                    ...
                    if ($this->getUserRole() == UC_ADMINISTRATOR || $this->getUserRole() == UC_MODERATOR) {
                        ...
                        if ($hotpotato['value'] == 'on') {
                            ...
                            if ($query_off == true && $query_user == true) {
                                $this->insertChatBotMessage($this->getChannel(),
                                    "[color=orange]Hot Potato:[/color] Hot potato is now [color=red]off[/color]");
                            } else {
                                $this->insertChatBotMessage($this->getPrivateMessageID(), "[color=red]Error:[/color] Database error");
                            }
                        } else {
                            $this->insertChatBotMessage($this->getPrivateMessageID(),
                                "[color=red]Error:[/color] Hot potato is already off, type !hot ON to turn it on");
                        }
                    } else {
                        $this->insertChatBotMessage($this->getPrivateMessageID(), "[color=red]Error:[/color] You do not have permission");
                    }
                ...
                case 'shout':
                    $this->insertChatBotMessage($this->getChannel(),
                        "[color=orange]Hot Potato:[/color] [b]It's starting starting soon![/b] Type /away to not play (you cannot change from away once the Hot Potato is on!)");
                    break;
                case 'help':
                    $this->insertChatBotMessage($this->getChannel(),
                        "When Hot Potato starts, a virtual hot potato is thrown at a random user and they have a time limit to pass it on before they lose");
                            ...
                            if ($usercount > 2 || ($usercount != 1 && $elim == 'on')) {
                                ...
                                if ($this->getIDFromName($this->getUserName()) == $hotuser['user'] ||
                                    ...
                                    if ($hotelim['value'] == 'on' && $hotuser['user'] == '0') {
                                        $this->insertChatBotMessage($this->getChannel(),
                                            "[color=orange]Hot Potato:[/color] The potato has been passed to [b]".$randuser_username."[/b] (".$throwtime." second".$plural." to throw, pass ".$hotcount['value'].", throw with !hot)");
                                    } else {
                                        // Announce it has been passed
                                        $this->insertChatBotMessage($this->getChannel(),
                                            "[color=orange]Hot Potato:[/color] ".$this->getUserName()." has passed the hot potato to [b]".$randuser_username."[/b] (".$throwtime." seconds to throw, pass ".$hotcount['value'].", throw with !hot)");
                                    }

                                    // Announce sudden death
                                    if ($hotcount['value'] == 20) {
                                        $this->insertChatBotMessage($this->getChannel(),
                                            "[color=orange]Hot Potato:[/color] Sudden death! 2 seconds inbetween throws!");
                                    } elseif ($hotcount['value'] == 30) {
                                        $this->insertChatBotMessage($this->getChannel(),
                                            "[color=orange]Hot Potato:[/color] [color=red]Super sudden death engaged! 1 second inbetween throws![/color]");
                                    }
                                    ...
                                    if (array_count_values($array) > 2) {
                                        ...
                                        if ($query == true) {
                                            ...
                                            $this->insertChatBotMessage($this->getChannel(),
                                                "[color=orange]Hot Potato:[/color] An ignited potato is thrown randomly at [b]".$randuser_username."[/b] (20 seconds to throw, throw with !hot)");
                                        } else {
                                            $this->insertChatBotMessage($this->getPrivateMessageID(), "[color=red]Error:[/color] Database error");
                                        }
                                    } else {
                                        $this->insertChatBotMessage($this->getPrivateMessageID(),
                                            "[color=red]Error:[/color] Must have more than 2 available players");
                                    }
                                } else {
                                    $this->insertChatBotMessage($this->getPrivateMessageID(), "[color=red]Error:[/color] You don't have the potato");
                                }
                                ...
                                $this->insertChatBotMessage($this->getChannel(),
                                    "[color=orange]Hot Potato:[/color] We have a winner! [b]".$this->getUsername()."[/b] was the winner after ".$hotcount['value']." passes!");
                                ...
                                $this->insertChatBotMessage($this->getPrivateMessageID(),
                                    "[color=red]Error:[/color] Hot Potato requires more than 2 players");
                            }
                        } else {
    ...
    case '!help':
        $this->insertChatBotMessage($this->getPrivateMessageID(),
            "These are my commands: !lick USER, !jump USER, !hug USER, !like IDEA USER, !flip A B, !quote, !image, !video, !randep, !time, !poke USER, !moon, !randomuser, !trivia HELP, !cmd, !away");
        break;
