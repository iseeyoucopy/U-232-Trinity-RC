...
class AJAXChat
{
    ...
    function addToOnlineList()
    {
        sql_query('INSERT INTO ajax_chat_online (
				...
				VALUES (
					...
					NOW(),
					'.sqlesc($this->ipToStorageFormat($_SERVER['REMOTE_ADDR'])).') ON DUPLICATE KEY UPDATE userID = userID') or sqlerr(__FILE__,
            __LINE__);
            ...
            if ($this->getUserRole() == UC_SYSOP) {
                ...
                if ($this->getUserRole() == UC_ADMINISTRATOR) {
                    ...
                    if ($this->getUserRole() == UC_MODERATOR) {
                        ...
                        if ($this->getUserRole() == UC_UPLOADER && $this->getConfig('allowUserMessageDelete')) {
                            ...
                            if ($this->getUserRole() == UC_VIP && $this->getConfig('allowUserMessageDelete')) {
                                ...
                                if ($this->getUserRole() == UC_POWER_USER && $this->getConfig('allowUserMessageDelete')) {
                                    ...
                                    if ($this->getUserRole() == UC_USER && $this->getConfig('allowUserMessageDelete')) {
    ...
    function insertCustomMessage($userID, $userName, $userRole, $channelID, $text, $ip = null, $mode = 0)
    {
        ...
        sql_query('UPDATE usersachiev SET dailyshouts=dailyshouts+1, weeklyshouts = weeklyshouts+1, monthlyshouts = monthlyshouts+1, totalshouts = totalshouts+1 WHERE id = '.sqlesc($userID)) or sqlerr(__FILE__,
            __LINE__);
    ...
    function getMessageCondition()
    {
        ...
        if ($this->getConfig('requestMessagesPriorChannelEnter') ||
            ($this->getConfig('requestMessagesPriorChannelEnterList') && in_array($this->getChannel(),
    ...
    function getTeaserMessageCondition()
    {
        ...
        $condition = 'channel = '.sqlesc($channelID).'
						...
						';
        if ($this->getConfig('requestMessagesPriorChannelEnter') ||
            ($this->getConfig('requestMessagesPriorChannelEnterList') && in_array($channelID,
                    $this->getConfig('requestMessagesPriorChannelEnterList')))) {
            $condition .= 'NOW() < DATE_ADD(dateTime, interval '.$this->getConfig('requestMessagesTimeDiff').' HOUR)';
        } else {
    ...
    function getLogsViewCondition()
    {
        ...
        switch ($this->getRequestVar('channelID')) {
            ...
            case '-1':
                ...
                break;
            default:
                if (($this->getUserRole() == UC_MODERATOR || !$this->getConfig('logsUserAccessChannelList') || in_array($this->getRequestVar('channelID'),
                            $this->getConfig('logsUserAccessChannelList')))
        ...
        if ($this->getRequestVar('search')) {
            if (($this->getUserRole() >= UC_MODERATOR) && strpos($this->getRequestVar('search'), 'ip=') === 0) {
                ...
                if (strpos($this->getRequestVar('search'), 'userID=') === 0) {
