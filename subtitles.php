    ...
    if ($action == "upload" || $action == "edit") {
        ...
        if ($action == "edit") {
            ...
            if ($id == 0) stderr($lang['gl_error'], $lang['gl_not_a_valid_id']);
                ...
                $arr = $res->fetch_assoc();
                if ($res->num_row() == 0) stderr($lang['gl_sorry'], $lang['subtitles_there_is_no_subtitle_with_that_id']);
...
<input type='submit' value='{$lang['gl_search']}' />&nbsp;<input type='button' onclick=\"window.location.href='subtitles.php?mode=upload'\" value='Upload' />
    ...
    if ($s) {
        $HTMLOUT.= "<tr><td style='border:none;'>{$lang['subtitles_search_result_for']} <i>'{$s}'</i><br />" . ($res->num_row() == 0 ? $lang['subtitles_nothing_found'] : "") . "</td></tr>";
    }
    $HTMLOUT.= "
</table>
<br />";
