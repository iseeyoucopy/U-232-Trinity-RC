...
function db_test()
{
    global $root, $TRINITY20;
    $out = '<fieldset><legend>Database</legend>';
    require_once ($root.'include/config.php');
    $mysqli_test = new mysqli($TRINITY20['mysql_host'], $TRINITY20['mysql_user'], $TRINITY20['mysql_pass'], $TRINITY20['mysql_db']);
    if (!$mysqli_test->connect_error) {
        $out.= '<div class="readable">Connection to database was made</div>';
        if ($mysqli_test->select_db($TRINITY20['mysql_db'])) {
...
function db_insert()
{
    ...
    $q = sprintf('/usr/bin/mysql -h %s -u %s -p%s %s < %sinstall/extra/'.$file, $TRINITY20['mysql_host'], $TRINITY20['mysql_user'], $TRINITY20['mysql_pass'], $TRINITY20['mysql_db'], $root); //== Linux
    //$q = sprintf('c:\AppServ\MySQL\bin\mysql -h %s -u %s -p%s %s < %sinstall/extra/'.$file,$TRINITY20['mysql_host'],$TRINITY20['mysql_user'],$TRINITY20['mysql_pass'],$TRINITY20['mysql_db'],$root);  //== Win - remember to set your path up correctly - atm its set for appserv
    exec($q, $o);
    if (!count($o)) {
